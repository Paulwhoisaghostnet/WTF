...
Release lane: `origin/main` at live commit `cb74cc1`
...
| Unified installer catalog | Live `https://wtfos.app/api/pasta/installers/catalog` returned unauthenticated `401` after PR #14 deployed commit `04b242a` and again on current live commit `cb74cc1`; `PASTA_LIVE_READINESS_ALLOW_BLOCKERS=1 npm run pasta:live-readiness` now verifies this route before reporting only WTF.ME blockers. | PROVEN |
