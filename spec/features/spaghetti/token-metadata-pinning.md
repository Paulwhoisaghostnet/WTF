# Spaghetti: Token metadata pinning

## Purpose

Token metadata pinning lets Pasta Protocol users move the Spaghetti workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for token metadata pinning.
...
- Validated state for token metadata pinning.
...
- Standalone validation: open Spaghetti, exercise token metadata pinning, assert visible status and no console-modal dependency.
