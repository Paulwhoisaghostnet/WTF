# Macaroni: Token media and metadata preparation

## Purpose

Token media and metadata preparation lets Pasta Protocol users move the Macaroni workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for token media and metadata preparation.
...
- Validated state for token media and metadata preparation.
...
- Standalone validation: open Macaroni, exercise token media and metadata preparation, assert visible status and no console-modal dependency.
