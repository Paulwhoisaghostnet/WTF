...
const PRODUCT_NAME = "Penne Studio";
...
function penneRoot() {
  return path.join(appRoot(), "penne");
}
...
function safeStaticPath(urlPath) {
  ...
  const fullPath = path.join(penneRoot(), rel);
  const root = penneRoot();
...
async function handleRequest(req, res) {
  ...
  if (req.method === "GET" && parsed.pathname === "/api/auth/user") {
    return sendJson(res, 401, {
      error: "Penne Desktop does not include wtfOS hosted resources.",
      native: true,
      product: "penne-studio",
    });
  }
  if (req.method === "GET" && parsed.pathname === "/api/auth/csrf-token") {
    return sendJson(res, 200, { csrfToken: "penne-desktop" });
  }
  if (req.method === "GET" && parsed.pathname === "/api/penne/installers") {
  ...
  if (
    ...
    return sendJson(res, 403, {
      error: "wtfOS hosted pinning, publishing, and preview processing are not available in Penne Desktop. Use Pinata or your own IPFS node.",
      native: true,
      product: "penne-studio",
    });
  }
...
if (!hasSingleInstanceLock) {
  ...
  app.whenReady().then(async () => {
    try {
      ...
      dialog.showErrorBox("Penne Studio failed to start", err.message || String(err));
      app.quit();
    }
