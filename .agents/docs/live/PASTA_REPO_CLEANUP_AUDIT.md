...
- Authoritative production source is current `origin/main` at `51ab323fb0e60b56ef15af5d32d06a306fff0d78` (`Merge pull request #13 from Paulwhoisaghostnet/codex/pasta-installer-audit-docs`).
- Live `https://wtfos.app/api/health` returned `status: "ok"`, `version.commitRef: "51ab323"`, `nodeEnv: "production"`, healthy database, and current mainnet chain defaults after the standalone installer audit fix deployment.
- Clean promotion branch `codex/pasta-installer-audit-docs` has been merged into `main` for the scoped Pasta installer/readiness evidence fix. PR #13 promoted the GitHub Actions API workflow-list audit fix plus refreshed live evidence docs; the current active branch is `codex/pasta-readiness-catalog-live` with follow-up read-only WTF.ME inventory/readiness hardening, repo cleanup gate integration, and unified installer-catalog readiness hardening described below.
- Historical Pasta release milestones remain ancestors of `51ab323`: Gnocchi/Ravioli/Rotini/Penne/Lasagna standalone installer lane commit `0558be0`, Spaghetti installer route commit `09fff2fb`, Spaghetti live verification `e080b89`, stale-worktree cleanup `21e1aca`, and Shadownet preflight `24cf9e2`.
- Deploy to Hetzner run `28521762196` and main Quality Gates run `28521762176` passed for `51ab323`, superseding the earlier standalone installer merge/deploy as the current live authority.
...
- 2026-07-01 follow-up verification from the clean `51ab323` worktree passed `npm run pasta:live-readiness:check`, `npm run pasta:standalone-installers:audit:check`, `PASTA_STANDALONE_INSTALLER_AUDIT_ALLOW_BLOCKERS=1 npm run pasta:standalone-installers:audit`, and `PASTA_LIVE_READINESS_ALLOW_BLOCKERS=1 npm run pasta:live-readiness`; the latter blocked only on missing dedicated WTF.ME publish credentials and missing live Pasta WTF.ME host proof. A later local diagnostic pass folds the read-only WTF.ME inventory prerequisite checklist, repo cleanup audit, and authenticated `/api/pasta/installers/catalog` route probe into the shared readiness gate; until this branch is deployed, `PASTA_LIVE_READINESS_ALLOW_BLOCKERS=1 npm run pasta:live-readiness` now also blocks on production returning `404` for `/api/pasta/installers/catalog` instead of the expected auth-protected `401`.
- 2026-07-01 cleanup-gate follow-up added `npm run pasta:repo-cleanup:audit` plus `npm run pasta:repo-cleanup:audit:check`; the audit passed from the active `51ab323` worktree and now machine-classifies Pasta-relevant branches/worktrees as active, promoted ancestors, or historical evidence unsafe to replay while ignoring Gamma/Beta noise. `npm run pasta:live-readiness` now invokes this audit by default so release-readiness runs fail on unclassified Pasta branch/worktree state unless explicitly skipped with `PASTA_LIVE_READINESS_CHECK_REPO_CLEANUP=0`.
...
| `.config/superpowers/worktrees/WTF/codex-spaghetti-installer-live` | `codex/pasta-readiness-catalog-live` / `51ab323` | 16 | Active Pasta readiness hardening checkout | Use as the current proof base for Pasta/WTF.ME readiness, repo-cleanup hardening, and unified installer-catalog readiness. Its local delta is the read-only WTF.ME inventory/prerequisite gate, executable cleanup audit hardening, authenticated suite/individual installer catalogue, and docs; full Pasta remains blocked by live WTF.ME host and pin-discovery proof, not by individual installer publication. |
...
| --- | ---: | --- | --- |
| `codex/pasta-readiness-catalog-live` | local working tree only | Valid ongoing work | Keep working here for read-only WTF.ME readiness, repo cleanup, and unified installer-catalog hardening; it starts at live `51ab323` and has no branch commits ahead yet before this local commit. |
...
Executable audit proof: `npm run pasta:repo-cleanup:audit` passed with `ok: true`, base `origin/main` at `51ab323fb0e60b56ef15af5d32d06a306fff0d78`, active ref `codex/pasta-readiness-catalog-live`, no blockers, `codex/pasta-live-readiness` and `origin/codex/pasta-live-readiness` classified as `historical_evidence_unsafe_to_replay`, `codex/pasta-installer-audit-docs`, `codex/spaghetti-installer-live`, and promoted Macaroni/IPFS branches classified as `promoted_ancestor`, and the active worktree classified as `valid_ongoing_worktree`.
...
- Pasta suite installer/download is now production-complete for the bundled native suite lane: `apps/pasta-suite-desktop`, the `pasta-suite-desktop-installers.yml` workflow, `/api/pasta/installers`, inventory coverage, package policy checks, GitHub release assets, production env, and the authenticated live verifier all passed for `pasta-suite-desktop-v1.0.0`. The current local follow-up adds `/api/pasta/installers/catalog` as the authenticated aggregate surface for "suite or any individual Pasta app"; it is source- and inventory-owned locally, but not production-complete until deployed and the readiness gate sees an unauthenticated `401` from `https://wtfos.app/api/pasta/installers/catalog`.
...
- The former `codex/pasta-live-readiness` proof branch has been mined/reworked into the current release lane. Future work should continue from the current `codex/pasta-readiness-catalog-live` worktree or a fresh branch from `origin/main`, not from the older broad proof branch, the merged `codex/pasta-installer-audit-docs` branch, or the now-promoted `codex/spaghetti-installer-live` branch.
