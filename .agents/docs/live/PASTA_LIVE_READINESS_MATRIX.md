...
Production publication helper: `npm run pasta:wtfme:live-publish` performs a dry-run with Pasta-specific credentials and prints the resolved user-site host; it only mutates production when `PASTA_WTFME_LIVE_PUBLISH=1` is set. `npm run pasta:wtfme:live-publish:check` locks the dry-run default, Pasta-scoped credentials, expected-host guard, CSRF writes, eligibility diagnostics, and post-publish TLS probe in source-policy coverage. `npm run pasta:wtfme:live-inventory` is the read-only production inventory probe for `/api/wtf-sites/my`, admin site listing, pin registry state, and TLS ask status, with `npm run pasta:wtfme:live-inventory:check` guarding that it does not call claim/save/publish APIs. A 2026-07-01 production dry-run with trusted-creator puppet `e2e_cookiemonster` authenticated successfully but exposed no claimable host because the underscore username is not a DNS label and the account lacks OAuth social, linked Bluesky, and active WTF DID state. A 2026-07-01 read-only inventory run with admin puppet `e2e_thecount` found no claimable admin-puppet host, `cobwebsaints.wtfos.me` as draft/TLS-allowed, `paulwhoisaghost.wtfos.me` as published/TLS-allowed, and `wtf-admin.wtfos.me` still denied as `handle not registered`.
