...
`LAW.W2/03` completes the final W client surgery pass. The W page no longer imports the old mutation hook, no client code on the active W surface calls W post/reply/like/repost/quote, follow, or Spaces endpoints, and the timeline panel exposes only read/open behavior while Gameshow Chat remains a read-only mirror. Verified 2026-05-18 with `npx tsx --test docs/constitutional-acceptance-policy.test.ts server/features/w/w-x-surgery-policy.test.ts server/features/w/timeline-stream.test.ts`, `npm run check -- --pretty false`, `npm run build`, `npm run test:e2e:inventory:coverage`, and `PORT=3413 HARNESS_PORT=3413 npx playwright test tests/playwright/w.spec.mjs`.
