    ...
    for (const row of artifactInventory) {
      for (let ordinal = 1; ordinal <= row.quantity; ordinal += 1) {
        ...
        if (row.spawner.kind === "tool") {
          nextItems.push(
            createDesktopItemForTool(row.spawner.tool, point.x, point.y, bounds, {
              sourceSku: row.sku,
          ...
          nextItems.push(
            createDesktopArtifactItem(row.spawner.itemKind, point.x, point.y, bounds, {
              sourceSku: row.sku,
              inventoryOrdinal: ordinal,
            })
          );
