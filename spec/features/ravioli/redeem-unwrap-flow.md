...
Redeem/unwrap flow lets Pasta Protocol users move the Ravioli workflow toward a valid token product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for redeem/unwrap flow.
...
- Validated state for redeem/unwrap flow.
...
- Standalone validation: open Ravioli, exercise redeem/unwrap flow, assert visible status and no console-modal dependency.
