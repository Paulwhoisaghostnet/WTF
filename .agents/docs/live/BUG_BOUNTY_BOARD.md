...
- Evidence:
  ...
  - The current release-lane pass adds `npm run pasta:live-readiness` plus `npm run pasta:live-readiness:check`. Audit mode proves live health and live static bundle/runtime markers, then reports the current blockers instead of allowing a vague "almost ready" claim.
