# Ravioli: Wrapped set provenance export

## Purpose

Wrapped set provenance export lets Pasta Protocol users move the Ravioli workflow toward a valid token product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for wrapped set provenance export.
...
- Validated state for wrapped set provenance export.
...
- Standalone validation: open Ravioli, exercise wrapped set provenance export, assert visible status and no console-modal dependency.
