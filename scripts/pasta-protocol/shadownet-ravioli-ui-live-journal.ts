...
export class RavioliUiLiveJournal {
  ...
  private planExtensionBoundarySha256 = "";
  private readonly nativeEffectivePlan: boolean;
  private readonly replayPostEvent86BoundarySha256: string;
  ...
  constructor(
    ...
    replayPostEvent86BoundarySha256 =
    ...
    this.replayPostEvent86BoundarySha256 = requireHash(
      replayPostEvent86BoundarySha256,
      "Ravioli replay post-event-86 boundary",
    );
  ...
  private applyPostEvent86PlanExtension(
    ...
    this.planExtensionBoundarySha256 = requireHash(
      boundarySha256,
      "Ravioli plan extension boundary",
    );
  ...
  /**
   * Generic integrity core for a caller that already trusts an event-86 SHA.
   * Operator continuations use the exact wrapper below, which supplies only
   * the frozen historical boundary.
   */
  appendPostEvent86PlanExtensionAgainstBoundary(
    requiredPreviousRecordSha256: string,
    ...
    const boundarySha256 = requireHash(
      requiredPreviousRecordSha256,
      "Ravioli required post-event-86 boundary",
    );
    return this.serialized(async () => {
      this.ensureWritable();
      ...
      if (this.planExtensionActive) {
        if (this.planExtensionBoundarySha256 !== boundarySha256) {
          throw new Error("Ravioli active plan extension boundary drift");
        }
        return Object.freeze({
          appended: false,
          eventIndex: 87,
  ...
  appendAuthenticatedPostEvent86PlanExtension(
    ...
    return this.appendPostEvent86PlanExtensionAgainstBoundary(
      RAVIOLI_UI_LIVE_POST_EVENT86_PLAN_EXTENSION
        .requiredPreviousRecordSha256,
      recordedAt,
    );
    ...
    if (phase === "PREPARED") {
      ...
      if (
        ...
        || record.previousRecordSha256
          !== this.replayPostEvent86BoundarySha256
      ) {
...
/**
 * Reopen against a separately trusted event-86 SHA. The exact operator opener
 * below remains closed over the frozen historical boundary.
 */
export async function openRavioliUiLiveJournalAgainstPostEvent86Boundary(
  journalRoot: string,
  requiredPreviousRecordSha256: string,
  ...
  const journal = new RavioliUiLiveJournal(
    resolved,
    intent,
    file.sha256,
    requiredPreviousRecordSha256,
...
export async function openRavioliUiLiveJournal(
  journalRoot: string,
): Promise<RavioliUiLiveJournal> {
  return openRavioliUiLiveJournalAgainstPostEvent86Boundary(
    journalRoot,
    RAVIOLI_UI_LIVE_POST_EVENT86_PLAN_EXTENSION
      .requiredPreviousRecordSha256,
  );
