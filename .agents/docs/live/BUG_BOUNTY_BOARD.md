...
- Evidence:
  ...
  - 2026-07-01 rerun: `npm run pasta:wtfme:live-publish:check` passed, while `npm run pasta:wtfme:live-check` still fails at the same production `handle not registered` gate.
  - 2026-07-01 all-puppet inventory: `npm run pasta:wtfme:live-inventory` authenticated all 12 available production puppets in read-only mode; every user returned `mySite=null`, no claimable host, invalid DNS-label username, missing OAuth social, and missing linked Bluesky/active WTF DID state.
  - Three elevated puppets can list the admin inventory, which contains only `cobwebsaints.wtfos.me` (`draft`, TLS allowed) and `paulwhoisaghost.wtfos.me` (`published`, TLS allowed). These are registered hosts but not Pasta publish authority without owner credentials or an intentional Pasta publish.
- Why it matters:
...
- Correction direction:
  ...
  - Added `scripts/pasta-protocol/wtfme-live-inventory.ts` and `npm run pasta:wtfme:live-inventory` so operators can safely inventory current-user site eligibility, admin-visible site state, pin registry binding, and TLS ask state before any production mutation.
