...
- Authoritative production source is current `origin/main` at `95075029328381dd104e144689e87b543d038ad4` (`Merge pull request #15 from Paulwhoisaghostnet/codex/pasta-live-verification-docs`).
- Live `https://wtfos.app/api/health` returned `status: "ok"`, `version.commitRef: "9507502"`, healthy mainnet chain defaults, and the unified Pasta installer catalog remains deployed and auth-protected on this live commit.
- Clean promotion branch `codex/pasta-installer-audit-docs` was merged into `main` for the scoped Pasta installer/readiness evidence fix. PR #13 promoted the GitHub Actions API workflow-list audit fix plus refreshed live evidence docs. PR #14 then promoted the read-only WTF.ME inventory/readiness hardening, repo cleanup gate integration, and unified installer-catalog readiness hardening. PR #15 promoted the post-PR #14 live verification evidence and cleanup-audit active-branch/remote classification fix, then the now-promoted `codex/pasta-live-verification-docs` branch was deleted.
- Historical Pasta release milestones remain ancestors of `9507502`: Gnocchi/Ravioli/Rotini/Penne/Lasagna standalone installer lane commit `0558be0`, Spaghetti installer route commit `09fff2fb`, Spaghetti live verification `e080b89`, stale-worktree cleanup `21e1aca`, Shadownet preflight `24cf9e2`, standalone audit fix `51ab323`, installer catalog gate `af9461f`, and live cleanup/catalog verification commit `e7967bd8`.
- Deploy to Hetzner run `28531403032` and main Quality Gates run `28531403138` passed for `9507502`. Earlier deploy `28529083020` and Quality Gates run `28529082794` passed for `cb74cc13`; deploy `28527900056` and Quality Gates run `28527900086` passed for `04b242a`, where the installer catalog first reached live production.
...
- 2026-07-01 follow-up verification from the clean `9507502` live state passed `npm run pasta:live-readiness:check`, `npm run pasta:standalone-installers:audit:check`, `npm run pasta:repo-cleanup:audit:check`, `npm run pasta:repo-cleanup:audit`, and `PASTA_LIVE_READINESS_ALLOW_BLOCKERS=1 npm run pasta:live-readiness`; the latter proves live health at `9507502`, the unified `/api/pasta/installers/catalog` route is live and auth-protected with unauthenticated `401`, all individual/suite installer release assets verify, the recorded Colander action proof verifies, and the only remaining blockers are missing dedicated WTF.ME publish credentials plus missing live Pasta WTF.ME host proof.
...
| `.config/superpowers/worktrees/WTF/codex-spaghetti-installer-live` | `codex/pasta-post-merge-cleanup-docs` / current branch head | 0 | Current cleanup documentation checkout | Use as the clean proof base for the post-prune audit refresh. PR #15 is promoted and its branch was deleted; full Pasta remains blocked by live WTF.ME host and pin-discovery proof, not by individual installer publication, catalog deployment, or repo cleanup. |
...
Executable audit proof: after the post-PR #15 prune, `npm run pasta:repo-cleanup:audit` passed with `ok: true`, base `origin/main` at `95075029328381dd104e144689e87b543d038ad4`, active ref `codex/pasta-post-merge-cleanup-docs`, no blockers, the current branch classified as `valid_ongoing_work` with a one-commit docs delta, `codex/pasta-live-readiness` / `origin/codex/pasta-live-readiness` classified as `historical_evidence_unsafe_to_replay`, the retained `codex-pasta-live-readiness` worktree classified as `historical_evidence_worktree`, and this cleanup-docs checkout classified as `valid_ongoing_worktree` with `dirty=0`.
