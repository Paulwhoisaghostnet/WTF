...
function getB58() {
  ...
  if (typeof lib.decode !== "function") {
    throw new Error("bs58check: cannot find decode function");
  }
...
function prefixMatch(data: Buffer, prefix: Buffer): boolean {
...
function b58decodeAny(encoded: string): { raw: Buffer; type: typeof KEY_TYPES[number] } | null {
  let decoded: Buffer;
  try {
    decoded = toBuffer(getB58().decode(encoded));
  } catch {
    return null;
  }
  for (const kt of KEY_TYPES) {
    if (decoded.length > kt.pkPrefix.length && prefixMatch(decoded, kt.pkPrefix)) {
...
function decodeSigAny(signature: string): Buffer | null {
  let decoded: Buffer;
  try {
    decoded = toBuffer(getB58().decode(signature));
  } catch {
    return null;
  }
  for (const kt of KEY_TYPES) {
    if (decoded.length > kt.sigPrefix.length && prefixMatch(decoded, kt.sigPrefix)) {
      return decoded.subarray(kt.sigPrefix.length);
    }
  }
  if (decoded.length > SIG_PREFIX.length && prefixMatch(decoded, SIG_PREFIX)) {
...
export function publicKeyToAddress(publicKey: string): string | null {
  ...
  return getB58().encode(Buffer.concat([parsed.type.addrPrefix, hash]));
}
