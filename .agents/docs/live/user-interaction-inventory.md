...
| Side quest rewards | Session/system/staff | Award quest XP; insert reward ledger row; show the signed-in reward account; auto-detect completed auto-verifiers; let the user claim verified WTF into the in-app ledger; keep EXP as an in-app-only score. | `xp.awarded`, `reward.ledger.created`, `reward.account.viewed`, `side_quest.reward_claimed`, `side_quest.reward_distributed` |
