...
const STORAGE_PROFILE = Object.freeze({
  mint: Object.freeze({ storageBase: 400, storagePerUnit: 900 }),
...
async function preflightMint(stage, amount) {
  ...
  const { c, estimate } = await MD.withRpcFallback(async () => {
    ...
    const limits = await MD.estimateWalletOp(
      ...
      {
        ...STORAGE_PROFILE.mint,
        gasPerUnit: 480_000,
        units: amount,
        throwOnRecoverableRpcError: true,
      }
    );
...
async function confirmWalletOperation(op, entrypoint, statusId, actionLabel) {
  ...
  try {
    ...
    const row = await MD.assertOperationApplied(op, {
      network: CFG.network,
      contractAddress: CFG.contract,
      entrypoint,
    });
    return { status: "confirmed", hash: submittedHash, source: "tzkt", row };
  } catch (e) {
...
async function mintOnce(c, priceMutez) {
  const op = await MD.sendWalletOp(
    ...
    { ...STORAGE_PROFILE.mint, gasPerUnit: 480_000, units: 1 }
  );
...
async function mintBatch(c, priceMutez, amount) {
  ...
  try {
    ...
    const op = await MD.sendWalletOp(
      ...
      { amount: total, mutez: true },
      { ...STORAGE_PROFILE.mint, gasPerUnit: 480_000, units: amount }
    );
    $("mintStatus").textContent = "minting… waiting for confirmation";
    await confirmWalletOperation(op, "mint", "mintStatus", "mint");
...
async function mint() {
  ...
  try {
    ...
    // the global storage snapshot while an unawaited refresh is still running.
    await refresh();
    stage = activeStage(new Date()) || stage;
    await updateWalletStatus(stage, { force: true });
    await reveal(ids);
