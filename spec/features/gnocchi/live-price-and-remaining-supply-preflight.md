# Gnocchi: Live price and remaining supply preflight

## Purpose

Live price and remaining supply preflight lets Pasta Protocol users move the Gnocchi workflow toward a valid token product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- The user can access `/tools/gnocchi` and the relevant app gate is enabled.
...
- User-entered configuration for live price and remaining supply preflight.
- Wallet/network context when the feature signs or reads chain state.
- Package, media, metadata, contract, or relationship data appropriate to Gnocchi.

## Outputs

- Validated state for live price and remaining supply preflight.
...
- Standalone validation: open Gnocchi, exercise live price and remaining supply preflight, assert visible status and no console-modal dependency.
