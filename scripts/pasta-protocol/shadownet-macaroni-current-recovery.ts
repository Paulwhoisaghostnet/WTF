...
const V1_SUBMITTED_CHECKPOINT_PHASES = Object.freeze([
...
const V1_SUBMITTED_CHECKPOINT_FILES = Object.freeze([
  "intent.json",
  ...V1_SUBMITTED_CHECKPOINT_PHASES.map((phase, index) =>
...
function assertCheckpointEventHash(
  event: JsonObject,
  options: { index: number; phase: string; previousSha256: string },
): void {
  assert.equal(event.schema, "pastaprotocol-macaroni-current-recovery-event@1");
  assert.equal(event.checkpointId, MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.checkpointId);
...
export function authenticateExactMacaroniPrewriteCheckpointDocuments(input: {
  ...
  assert.equal(
    ...
    MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.recoveredPrefixEventSha256,
    "Macaroni recovered-prefix event file hash drift",
  );
  assert.equal(
    sha256(input.signersAuthenticatedEventBytes),
    MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.signersAuthenticatedEventSha256,
  ...
  assert.equal(intent.createdAt, MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.createdAt);
  assert.equal(intent.runId, MACARONI_CURRENT_RECOVERY_RUN_ID);
  assert.equal(intent.checkpointId, MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.checkpointId);
  ...
  assertCheckpointEventHash(recoveredPrefixEvent, {
    ...
    previousSha256: MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.intentSha256,
  });
  ...
  assertCheckpointEventHash(signersAuthenticatedEvent, {
    index: MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.lastEventIndex,
    phase: "SIGNERS_AUTHENTICATED",
    previousSha256: MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.recoveredPrefixEventSha256,
  });
  return {
    intent,
    recoveredPrefixEvent,
    signersAuthenticatedEvent,
    lastEventFileSha256: MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.signersAuthenticatedEventSha256,
  };
}
...
export function authenticateMacaroniV1SubmittedCheckpointDocuments(input: {
  ...
  assert.equal(input.eventBytes.length, V1_SUBMITTED_CHECKPOINT_PHASES.length);
  assert.equal(
    sha256(input.intentBytes),
    MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.intentSha256,
  ...
  let previousSha256: string = MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.intentSha256;
  const events = input.eventBytes.map((bytes, index) => {
    ...
    assertCheckpointEventHash(event, {
      index: index + 1,
      phase: V1_SUBMITTED_CHECKPOINT_PHASES[index],
      previousSha256,
    });
  ...
  assert.equal(
    previousSha256,
    MACARONI_V1_SUBMITTED_EVENT_FILE_SHA256,
  ...
  static async reopenV1Submitted(
    ...
    return {
      checkpoint: new MacaroniRecoveryCheckpoint(
        ...
        MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.intentSha256,
        {
          eventIndex: V1_SUBMITTED_CHECKPOINT_PHASES.length,
          previousSha256: authenticated.lastEventFileSha256,
        },
      ),
      createdAt: MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.createdAt,
