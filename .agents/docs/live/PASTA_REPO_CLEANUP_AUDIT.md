...
- Production credential discovery on 2026-07-01 found the available puppet users authenticate but do not provide a Pasta publish identity: `e2e_cookiemonster` has a wallet and can issue a WTF DID, but has an underscore username, no OAuth social, no linked Bluesky, and no active WTF DID; admin read-only inspection found only `cobwebsaints.wtfos.me` as draft and `paulwhoisaghost.wtfos.me` as published.
...
1. Use `.agents/docs/live/PASTA_LIVE_READINESS_MATRIX.md` as the live-push checklist, keep `npm run pasta:wtfme:live-publish:check` green, provision or identify a DNS-valid Pasta production user with the required wallet/social/DID state, then run `npm run pasta:wtfme:live-publish` in dry-run mode before any live mutation.
