# Gnocchi: Export/recovery manifest

## Purpose

Export/recovery manifest lets Pasta Protocol users move the Gnocchi workflow toward a valid token product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for export/recovery manifest.
...
- Validated state for export/recovery manifest.
...
- Standalone validation: open Gnocchi, exercise export/recovery manifest, assert visible status and no console-modal dependency.
