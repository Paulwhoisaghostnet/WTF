export const CORE_BEHAVIOR_ASSERTIONS = [
  ...
  },
  {
    ...
    verificationCommand:
      "npm run contract:macaroni-v3:compile && npx tsx --test server/features/macaroni/secure-allocation.test.ts server/routes/macaroni-policy.test.ts shared/pasta-protocol/foundation.test.ts",
    userVisibleAssertion:
      "A creator can select Macaroni V3 with immediate or delayed automatic reveal, keep each final token metadata CID plus private nonce out of pre-sale chain storage, commit a cryptographically shuffled edition allocation before opening the sale, and reveal token metadata only after a paid claim has been assigned; collector pages cannot choose a token, metadata CID, or reveal secret, and Colander recognizes the V3 contract as Macaroni-owned.",
    durableSideEffectAssertion:
      "The automatic service uses Node's cryptographic random generator for an unbiased Fisher-Yates edition shuffle, seals every slot with a fresh 32-byte nonce and SHA-256 commitment, encrypts the private allocation manifest, and returns only opaque slot commitments for creator approval; the compiled V3 contract blocks minting until every slot is committed and inventory is finalized, records paid claims sequentially, rejects unauthorized, nonsequential, malformed, or mismatched settlement proofs, and permits neither the buyer nor the creator to substitute a token after finalization without the hidden slot nonce. The operator settles claims automatically after mint confirmation in both reveal modes, reveals metadata immediately or after the configured delay, and the scheduler provides durable fallback; exported collector configuration and pre-sale chain storage omit final token metadata and artifact CIDs.",
  },
  {
