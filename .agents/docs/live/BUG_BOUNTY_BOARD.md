...
- Correction:
  ...
  - Extended the Playwright harness to seed published user-site pages and serve them from a `*.wtfos.me` public host with user-site CSP/opener headers and `wtf_site.public.viewed` event logging.
...
- Residual risk:
  - This is branch-level host-mapped proof, not production live DNS/TLS, user publish UI, hosted pinning/recovery, or signed hosted-page mint/purchase proof.
