...
export const WTF_STANDARD_BROWSER_ROUTES: WtfBrowserAccessRoute[] = [
  ...
  { path: "/mail", title: "Inbox", access: "browser-session", purpose: "Central messaging hub for Inbox Mail, WIM, Studio conversations, notifications, and normalized comms cards." },
  ...
  { path: "/dedrooms", title: "DedRooms", access: "browser-session", purpose: "Persistent authored-room MUD world with shared room presence, anchored map reveal, stateful NPCs, resources, alliances, and the Search for the Green Room intro campaign.", appGate: "dedrooms" },
...
export const WTF_STANDARD_API_ROUTES: WtfApiAccessRoute[] = [
  ...
  { method: "GET", path: "/api/dedrooms/state", access: "browser-session", purpose: "Read the signed-in player's DedRooms state, placed room, character sheet, map reveal, transcript, resources, inventory, and campaign mode.", appGate: "dedrooms" },
  { method: "POST", path: "/api/dedrooms/command", access: "browser-session", purpose: "Resolve one authoritative DedRooms MUD command for the signed-in player.", appGate: "dedrooms" },
  { method: "GET", path: "/api/dedrooms/history", access: "browser-session", purpose: "Read the signed-in player's DedRooms transcript and room-visible events.", appGate: "dedrooms" },
  { method: "GET", path: "/api/dedrooms/admin/content", access: "role-gated-session", purpose: "Admin DedRooms campaign/content editor overview with seeded world diagnostics.", appGate: "dedrooms" },
  { method: "POST", path: "/api/dedrooms/admin/content", access: "role-gated-session", purpose: "Admin publish/update DedRooms room, NPC, item, resource, event, dialogue, anchor, or campaign content records.", appGate: "dedrooms" },
  { method: "PATCH", path: "/api/dedrooms/admin/campaign", access: "role-gated-session", purpose: "Admin update Green Room intro campaign mode, target departures, and shared unlock progress inside DedRooms.", appGate: "dedrooms" },
  ...
  { method: "GET", path: "/api/mail/messages", access: "browser-session", purpose: "List the signed-in user's Inbox Mail messages." },
  { method: "GET", path: "/api/mail/messages/:id", access: "browser-session", purpose: "Read one owned Inbox Mail message." },
  { method: "POST", path: "/api/mail/send", access: "browser-session", purpose: "Send mail from an owned Inbox Mail address." },
