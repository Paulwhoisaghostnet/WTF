# CH-EASE: Single-token package export

## Purpose

Single-token package export lets Pasta Protocol users move the CH-EASE workflow toward a valid package outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for single-token package export.
...
- Validated state for single-token package export.
...
- Standalone validation: open CH-EASE, exercise single-token package export, assert visible status and no console-modal dependency.
