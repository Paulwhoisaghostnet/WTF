...
function contractForApp(app: string): PastaWtfmeProofContract {
...
function buildTokenMetadataItem(contract: PastaWtfmeProofContract): PastaPinSourceItem {
  const tokenId = contract.tokenId ?? "registry";
  const payload = {
...
function buildRelationshipMetadataItem(contract: PastaWtfmeProofContract): PastaPinSourceItem {
  const payload = {
    schemaVersion: PASTA_PINNING_SCHEMA_VERSION,
    product: "pasta-protocol",
    app: contract.app,
...
export function buildPastaPublishPinningProof(input: PastaPublishPinningProofInput) {
  ...
  const manifestPayload = {
    ...
    coverage: {
      ...
      metadataPinning:
        items.some((item) => item.kind === "token_metadata") &&
