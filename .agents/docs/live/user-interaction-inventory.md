...
| Social | `/messages`, `/messages/dms/:id`, `/mail`, `/digest`, `/skywire`, `/tz2at`, `/wim`, `/aim`, `/dear-diary`, `/messageboard`, `/w`, `/w/post/:id`, `/w/chat`, `/w/groupchat/:id`, `/chat`, `/chat/:id`, `/dicksword`, `/i-hate-telegram`, `/profile`, `/wtf-subdomains`, `/user/:username`. |
| Desktop OS | Desktop shell, Start Menu, taskbar, desktop icons, command palette, `/mission-control`, `/command-palette`, `/recovery-mode`, `/file-manager`, `/settings`, `/browser`, `/browser-boundaries`, `/terminal`, `/theme-builder`, `/notification-center`, `/desktop-settings`, `/task-manager`, desktop pet tray, desktop artifact layer, screen saver, custom cursor, MCP pairing. |
...
| Desktop icons, shortcuts, and Start Menu app gates | Session/public gates/time out blocked/admin app gates | Open enabled app icons and registry-derived Start Menu entries organized as Apps, domain categories, account/system items, and Browse; hide app icons, Start Menu entries, shortcuts, and command-palette launch commands from signed-in `time_out` accounts and from disabled app gates; use right-click or Shift-click Windows 95 context menus; drag enabled Start Menu items to the desktop to create route shortcuts; open/move/delete shortcuts; move/reposition native icons; reset layout; stale shortcut/direct-route launches for disabled apps render the admin-disabled failure state instead of running the app. | `desktop.icon.opened`, `desktop.icon.moved`, `desktop.icon_layout.reset`, `desktop.context_menu.opened`, `desktop.shortcut.created`, `desktop.shortcut.opened`, `desktop.shortcut.moved`, `desktop.shortcut.deleted`, `desktop.app.disabled_by_admin` |
...
| WIM | Session | Open an AOL-style instant messenger roster of WTF users; see logged-in users with online indicators; manage a browser-local friends list with collapsible friends/online/all-user/recent-direct-chat groups; double-click a WTF user to open or create that user's direct DM-backed chat history; send through the existing DM route without creating a parallel message backend. Studio project rooms stay out of the WIM buddy roster. Legacy `/aim` routes into this same app. | `wim.chat.opened`, `wim.friend.added`, `wim.message.sent`, `dm.message.sent` |
...
| Skywire AT Protocol bridge | Session/system | Open the native Skywire app; hand off new-account creation to the official Bluesky signup flow; connect an existing Bluesky/AT Protocol account through popup-capable OAuth that first asks the user to choose Be Safe, Be Social, Be Heard, or Be Bold permissions plus an explicit Bluesky chat/DM add-on; persist requested tier, requested scope, granted scope, DID/handle/profile/PDS, and encrypted OAuth/credential material server-side so normal page refreshes and deploys restore the AT session; gate Skywire actions by granted scope and surface reconnect/upgrade-required state instead of leaking raw SDK restore or upstream forbidden errors; default connected users into a Bluesky-style home timeline backed by `app.bsky.feed.getTimeline`; click a home-feed actor handle/avatar to open that actor's dedicated author-only feed with cursor pagination; read the official WTFgameshow account tab through `app.bsky.feed.getAuthorFeed`; read a curated Tezos feed composed only from official Tezos platform/community author feeds; read normalized Bluesky feed cards with authors, timestamps, embeds, metrics, viewer state, source links, and cursor pagination; recommend other WTF users who connected AT identities while excluding self-follow; list actors the connected account follows through `app.bsky.graph.getFollows`, compare the user's graph with other Skywire users' follow graphs to suggest unfollowed actors, select any Discover actor into the dedicated Actor Feed tab, and follow actors; search actors through public AppView, inspect author feeds, and follow actors; distinguish AT-compliant handles from Tezos aliases; bridge the user's preferred Tezos domain identity into handle claims; claim or verify WTF-hosted/DNS/HTTPS handles; serve public `/.well-known/atproto-did`; read search/notification feeds; compose, like, repost, reply, receive notifications, and claim existing posts for challenge automation; publish WTF-native Skywire Signal records into the user's AT repo for portable quest/drop/proof/broadcast state beyond Bluesky's app surface; use Bluesky compose intent fallback without auto-posting. | `atproto.permission_tier.selected`, `atproto.chat_permission.toggled`, `atproto.external_signup.opened`, `atproto.account.linked`, `atproto.account.unlinked`, `atproto.profile.updated`, `atproto.actor.searched`, `atproto.actor.followed`, `atproto.handle.claimed`, `atproto.handle.verified`, `atproto.signal.published`, `atproto.post.created`, `atproto.post.claimed`, `atproto.post.replied`, `atproto.post.liked`, `atproto.post.reposted`, `atproto.notification.received` |
| tz2at identity proof, WTFOS PDS spine, and ecosystem analytics | Session/system | Open the native tz2at app; connect an AT Protocol DID with only the base `atproto` scope; import public tzbsky wallet proofs by reading `com.tzbsky.cryptoAddress/self` from the user's PDS without asking for new write approval; list locally verified WTF Tezos and Etherlink wallets; request the separate `repo:xyz.tz2at.identity.walletLink` scope only when the user chooses to publish a portable wallet-link proof; show the WTFOS PDS spine as a separate service for linked WTFOS DID/repo storage under `app.wtfos.*`; request a WTFOS repo only when the PDS is configured; provision the linked WTFOS repo when PDS health, invite policy, and provisioning flags allow it; write `app.wtfos.identity.link` to the WTFOS repo; keep canonical repo writes limited to allowlisted proofs; persist imported, published, requested, and provisioned state; normalize linked-wallet Tezos chain activity into `blockchain.tezos.*` SystemEvents so challenge, side quest, and reward automation can react to on-chain actions; enqueue `app.wtfos.activity.event` records for both the primary WTFOS repo and the user's linked WTF DID repo; preview wallet activity and search read-only tz2at replay/firehose events by type, chain, address, wallet, contract, marketplace, token, operation hash, and block range without writing chain activity into the user's canonical repo; aggregate live tz2at AT Protocol PDS repo records into a read-only ecosystem analytics suite showing PDS inventory, operator brief insights, record-family freshness, scoped host/network/collection segmentation, ecosystem lanes, largest value flows, aggregated liquidity routes, entity drilldown, analytics-to-firehose handoff, active addresses, contract usage, FA2 movement, marketplace liquidity, token/group activity, value-adder/value-extractor leaderboards, and built-in TzKT-labeled CEX custody classification, unclassified custody-candidate surfacing, and configurable CEX inflow/outflow extensions. | `tz2at.identity.viewed`, `tz2at.tzbsky.imported`, `tz2at.wallet_link.published`, `tz2at.wtfos_pds.requested`, `tz2at.wtfos_pds.provisioned`, `blockchain.tezos.token_transfer_in`, `blockchain.tezos.token_transfer_out`, `blockchain.tezos.token_mint`, `blockchain.tezos.contract_call`, `wtfos.atproto_outbox.enqueued`, `wtfos.atproto_outbox.published`, `tz2at.firehose.previewed`, `tz2at.firehose.searched`, `tz2at.ecosystem.analytics_viewed`, `tz2at.ecosystem.analytics_refreshed`, `tz2at.ecosystem.scope_changed`, `tz2at.ecosystem.entity_selected`, `tz2at.ecosystem.entity_scoped`, `tz2at.firehose.entity_opened`, `app.interaction.tracked` |
...
| Rat Race | Session/wallet | View urgency-ranked Tezos edition cards sourced from tz2at/local sale events; tune card limit, recent-sales window, minted-age window, minimum sold-through percentage, and minimum recent-buy count; inspect sold-through, active supply, floor, estimated time-to-sellout, source diagnostics, and near-miss rejection reasons; open the parent marketplace listing; create a direct wallet purchase intent for allowlisted external marketplace contracts without hiding the parent market option. | `rat_race.viewed`, `rat_race.card.opened`, `rat_race.purchase_intent.created`, `blockchain.tezos.token_sale` |
