...
- Production credential discovery on 2026-07-01 found the available puppet users authenticate but do not provide a Pasta publish identity: `e2e_cookiemonster` has a wallet and can issue a WTF DID, but has an underscore username, no OAuth social, no linked Bluesky, and no active WTF DID. A read-only `e2e_thecount` inventory run found the admin puppet also has no claimable host for the same DNS/social/DID reasons, while `cobwebsaints.wtfos.me` is draft/TLS-allowed, `paulwhoisaghost.wtfos.me` is published/TLS-allowed, and `wtf-admin.wtfos.me` remains denied as `handle not registered`.
...
- Live inventory evidence: a 2026-07-01 read-only run with production admin puppet `e2e_thecount` returned `mySite=null`, `nextRequiredHost=null`, `hasWallet=true`, `canIssueWtfDid=true`, and the same invalid DNS label plus missing OAuth/social/DID reasons. The admin list returned only `cobwebsaints.wtfos.me` (`draft`, home page, TLS allowed) and `paulwhoisaghost.wtfos.me` (`published`, home plus `airporters-vol-1`, TLS allowed); `wtf-admin.wtfos.me` still returned TLS ask `403` / `handle not registered`.
