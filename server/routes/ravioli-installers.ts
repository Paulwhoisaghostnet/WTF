...
const INSTALLER_PLATFORMS = [
  {
    key: "macos",
    label: "macOS",
    env: "RAVIOLI_INSTALLER_MACOS_URL",
    sha256Env: "RAVIOLI_INSTALLER_MACOS_SHA256",
    fileName: "Ravioli-Studio.dmg",
  },
  {
    key: "windows",
    label: "Windows",
    env: "RAVIOLI_INSTALLER_WINDOWS_URL",
    sha256Env: "RAVIOLI_INSTALLER_WINDOWS_SHA256",
    fileName: "Ravioli-Studio.exe",
  },
  {
    key: "raspberry-pi",
    label: "Raspberry Pi",
    env: "RAVIOLI_INSTALLER_RASPBERRY_PI_URL",
    sha256Env: "RAVIOLI_INSTALLER_RASPBERRY_PI_SHA256",
    fileName: "ravioli-studio-arm64.deb",
  },
] as const;
...
router.get("/api/ravioli/installers", isAuthenticated, (_req, res) => {
  const version = String(process.env.RAVIOLI_INSTALLER_VERSION || "").trim();
  return res.json({
    ok: true,
    product: "ravioli",
