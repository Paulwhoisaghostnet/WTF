...
| WTF-BB-359 | Fixed | Codex WTF LIVE game-room hotfix | 2026-07-08 | WTF LIVE / game room settings and Jackbox controls | P1 | 13 | 6 | 3 | 5 | 1 | Game room settings repair migration drops the old anonymous and explicit room_kind checks, re-adds `room|game|stage`, and backfills existing game rooms; create-room errors are sanitized; Jackbox host buttons now deep-link to the game room with `hostApp`, auto-join the manager, start apphost in-room, attach the hosted WebRTC stream to WTF LIVE screen sharing, and keep input on the room control surface. Local verification passed focused WTF LIVE policy tests, TypeScript, build, inventory coverage, focused WTF LIVE Playwright game-room smoke, and full inventory E2E (`592 passed`); production deploy verification pending |
