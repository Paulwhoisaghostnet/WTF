...
Evidence snapshot: the latest recorded production verification is PR #25 live cleanup-hardening commit `7657a3b`, with Deploy to Hetzner run `28561834866`, main Quality Gates run `28561834903`, live health reporting `commitRef:"7657a3b"`, `npm run pasta:repo-cleanup:audit` passing against `origin/main` after the promoted PR branch was pruned, and `PASTA_LIVE_READINESS_ALLOW_BLOCKERS=1 npm run pasta:live-readiness` proving all non-WTF.ME Pasta surfaces while still blocking only on dedicated WTF.ME credentials and `PASTA_WTFME_LIVE_HOST`. The current evidence-refresh branch also hardens cleanup-audit classification for refs that disappear mid-audit and reruns strict final-launch mode with repo cleanup passing. This is a production evidence snapshot plus branch validation, not a full Pasta launch claim.
...
| Unified installer catalog | Live `https://wtfos.app/api/pasta/installers/catalog` returned unauthenticated `401` after PR #14 deployed commit `04b242a`, on `cb74cc1`, on `9507502`, during the PR #16 verification snapshot at `2a1977e`, during the PR #17 verification snapshot at `26c60cd`, in the PR #22 production evidence snapshot at `984b3f5`, in the PR #23 production evidence snapshot at `92bb731`, and in the PR #25 production evidence snapshot at `7657a3b`; `PASTA_LIVE_READINESS_ALLOW_BLOCKERS=1 npm run pasta:live-readiness` verifies this route before reporting only WTF.ME blockers. | PROVEN |
...
| Pasta live-readiness gate | `npm run pasta:live-readiness:check` and `npm run pasta:repo-cleanup:audit:check` passed. Current branch policy coverage now includes the disappearing-ref guard and passes 9/9. In the latest PR #25 production evidence snapshot, `PASTA_LIVE_READINESS_ALLOW_BLOCKERS=1 npm run pasta:live-readiness` proved live `wtfos.app` health at commit `7657a3b`, repo cleanup, Taquito 25 markers across Macaroni plus six Pasta publisher bundles, `window.MD`/handoff/runtime markers across the six Pasta shared runtimes, protected installer manifests and public release assets for Macaroni Desktop, Pasta Suite Desktop, Spaghetti Desktop, Gnocchi Desktop, Ravioli Desktop, Rotini Desktop, Penne Desktop, and Lasagna Desktop, the auth-protected `/api/pasta/installers/catalog` route, and recorded Colander Shadownet action operation `oo2qtySsskwgYE41BAvN2jxYpvi1L8zugNwyk1JHXUWbYCj8P3h` without signer execution. Post-merge cleanup also proved the squash-merged PR #25 source branch had no file delta against `origin/main` before the branch was pruned locally and remotely. Current branch reruns `PASTA_LIVE_READINESS_FINAL_LAUNCH=1 npm run pasta:live-readiness`; strict mode passes repo cleanup/static/installer/catalog/Colander checks and exits `1` only on the two WTF.ME blockers. `PASTA_LIVE_READINESS_FINAL_LAUNCH=1` keeps blockers fatal even if `PASTA_LIVE_READINESS_ALLOW_BLOCKERS=1` is accidentally set and refuses disabled repo-cleanup/static/installer/Colander/WTF.ME probes. The gate still reports blockers for missing local WTF.ME publish credentials plus missing `PASTA_WTFME_LIVE_HOST`. Source policy requires supplied credentials to pass a forced non-writing `pasta:wtfme:live-publish` dry-run, bound to `PASTA_WTFME_LIVE_HOST` when present. With `PASTA_WTFME_LIVE_HOST=paulwhoisaghost.wtfos.me PASTA_WTFME_LIVE_CHECK_PINS=0`, it also proves the nearest known host still fails on missing `data-pasta-hosted-page="landing"`. | PARTIAL |
...
6. `PASTA_LIVE_READINESS_FINAL_LAUNCH=1 npm run pasta:live-readiness` exits successfully, with no blocker-allow mode and no disabled production probes.
...
3. Rerun `PASTA_LIVE_READINESS_FINAL_LAUNCH=1 npm run pasta:live-readiness` without blocker-allow mode and require it to pass before claiming the Pasta lane is production-ready.
