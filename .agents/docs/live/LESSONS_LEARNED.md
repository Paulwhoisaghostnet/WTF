...
**Rule**: Give high-frequency interactive endpoints (`/api/apphost/*`) their own rate limiter keyed by user session, mounted after auth, and exempt them from the generic `/api/*` quota — never let input events and auth reads share a bucket. Always start remote `<video>` muted and call `play()` immediately (muted autoplay is unconditionally allowed), then unmute on the first real user gesture; add a zero-frames-decoded watchdog that renegotiates the stream instead of waiting for a manual refresh. For remote app sessions, prefer the app's native cursor: stream it (`show-pointer=true`), hide the local/custom cursor over the play surface, and use pointer lock (Esc to release) translating relative movement to absolute remote coordinates. This supersedes the earlier "keep show-pointer=false" guidance, which only made sense while the local cursor was the primary pointer.
...
## 2026-07-08 - WTF LIVE game rooms own the hosted game screen, and generated CHECK names matter

**What happened**: Game room creation could succeed but still show a raw failed-query error because production still had the anonymous `room_kind IN ('room','stage')` CHECK constraints generated by the original inline migration, while the follow-up migration only dropped the later explicit names. At the UX layer, Jackbox buttons opened or targeted the standalone Applications play route, so the game was not actually hosted inside the room that started it.
