...
const INSTALLER_PLATFORMS = [
  {
    key: "macos",
    label: "macOS",
    env: "LASAGNA_INSTALLER_MACOS_URL",
    sha256Env: "LASAGNA_INSTALLER_MACOS_SHA256",
    fileName: "Lasagna-Studio.dmg",
  },
  {
    key: "windows",
    label: "Windows",
    env: "LASAGNA_INSTALLER_WINDOWS_URL",
    sha256Env: "LASAGNA_INSTALLER_WINDOWS_SHA256",
    fileName: "Lasagna-Studio.exe",
  },
  {
    key: "raspberry-pi",
    label: "Raspberry Pi",
    env: "LASAGNA_INSTALLER_RASPBERRY_PI_URL",
    sha256Env: "LASAGNA_INSTALLER_RASPBERRY_PI_SHA256",
    fileName: "lasagna-studio-arm64.deb",
  },
] as const;
...
router.get("/api/lasagna/installers", isAuthenticated, (_req, res) => {
  const version = String(process.env.LASAGNA_INSTALLER_VERSION || "").trim();
  return res.json({
    ok: true,
    product: "lasagna",
