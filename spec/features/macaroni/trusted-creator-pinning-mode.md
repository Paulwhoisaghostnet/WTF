# Macaroni: Trusted creator pinning mode

## Purpose

Trusted creator pinning mode lets Pasta Protocol users move the Macaroni workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for trusted creator pinning mode.
...
- Validated state for trusted creator pinning mode.
...
- Standalone validation: open Macaroni, exercise trusted creator pinning mode, assert visible status and no console-modal dependency.
