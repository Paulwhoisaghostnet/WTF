...
| WTF-BB-332 | Verified | Codex Pasta cleanup/status continuation | 2026-07-01 | Repo hygiene / Pasta stale worktrees | P2 | 9 | 12 | 1 | 3 | 2 | Stale `WTF-pasta-deploy` checkout is gone; promoted Spaghetti/Macaroni/IPFS/catalog branches and two clean promoted worktrees were pruned after live `9507502`; current cleanup audit keeps only `codex/pasta-live-readiness` and `wip/full-working-tree-20260617` as historical material unsafe to replay, and now runs inside `pasta:live-readiness` by default |
...
- Evidence:
  ...
  - 2026-07-01 branch recheck after PR #15: promoted Spaghetti, Macaroni, IPFS, installer-audit, and catalog branches were proven ancestors of `origin/main` and pruned locally/remotely where present; `codex/pasta-live-readiness` is `44` current-main commits behind and `22` historical commits ahead, and `wip/full-working-tree-20260617` remains an old backup over a much older base.
  - Two-dot replay checks showed `origin/main..codex/pasta-live-readiness` would touch 123 files with 30,149 deletions, including deletion of newer installer apps/workflows, live-readiness gates, Colander action proof tooling, and proof reports; `origin/main..wip/full-working-tree-20260617` would touch 807 files with 194,914 deletions, including current installer apps, workflows, Pasta reports, spec files, and live readiness tooling.
  - 2026-07-01 cleanup gate pass after pruning: `npm run pasta:repo-cleanup:audit` returned `ok: true`, classified only `codex/pasta-live-readiness`/`origin/codex/pasta-live-readiness` as historical evidence unsafe to replay, the retained `codex-pasta-live-readiness` worktree as historical evidence, and the active cleanup-docs checkout as valid ongoing work; it reported no unknown Pasta branches or worktrees.
