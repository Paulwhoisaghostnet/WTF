# Ravioli: Relationship metadata stamping

## Purpose

Relationship metadata stamping lets Pasta Protocol users move the Ravioli workflow toward a valid token product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- The user can access `/tools/ravioli` and the relevant app gate is enabled.
...
- User-entered configuration for relationship metadata stamping.
- Wallet/network context when the feature signs or reads chain state.
- Package, media, metadata, contract, or relationship data appropriate to Ravioli.

## Outputs

- Validated state for relationship metadata stamping.
...
- Standalone validation: open Ravioli, exercise relationship metadata stamping, assert visible status and no console-modal dependency.
