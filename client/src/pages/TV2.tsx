/* ------------------------------------------------------------------ */
/*  TV2 — Hidden Experimental Clone                                     */
/*                                                                     */
/*  Forked from TV.tsx on 2026-04-22 as a private scratch copy so      */
/*  engineers can try changes without rewriting the canonical TV page. */
/*  Registered at /tv2 in client/src/App.tsx WITHOUT a start-menu or   */
/*  desktop icon entry, so it never surfaces in the normal UI.         */
/*  Shares the same /api/tv/* backend as TV — backend fixes apply to   */
...
type StreamPayload = {
  ...
  baseCadence?: number;
  /**
   * Daypart descriptor from the server (see `daypartForMs` in
   * `server/routes/tv.ts`).  Optional because older clients talking
   * to fresh servers — or vice versa — shouldn't hard-fail the
   * stream query.  Consumer code must tolerate `undefined`.
   */
  daypart?: {
    name:
      | "late_night"
      ...
      | "evening";
    displayName: string;
    preferredCategory: "personal" | "community" | null;
...
/* ------------------------------------------------------------------ */
/*  Skip-notice banner (TV2 addition).                                  */
/*                                                                      */
/*  The audit's error-state finding is that the TV used to mask every  */
/*  failure behind vibe-y static — if a clip was blocked, oversized,   */
/*  or rejected by the cache the viewer got a lava lamp where content  */
/*  should have been.  This banner gives the viewer a short, readable  */
/*  acknowledgement ("Skipping broken clip…") so they know the channel */
/*  is moving on rather than freezing.  Render-only, no layout impact  */
/*  on the video — z-index puts it above the CRT stack but below the   */
/*  chrome overlays.                                                    */
/* ------------------------------------------------------------------ */
...
// ─────────────────────────────────────────────────────────────────
// MTV-style broadcast furniture
// ─────────────────────────────────────────────────────────────────
// These overlays live on top of the video surface but never inside
// fullscreen-only modes: the parent component keeps them gated on
// `screenView === "tv"` so the OFF / channel-menu views don't bleed
// broadcast chrome through.
//
// Every item uses `pointer-events: none` so clicks fall through to
// the underlying media element, and z-indexes are chosen to stack
// below the CRT glass overlay (`z-index: 20`) but above the video
// and the existing skip banner.

// Keyframes defined once at module scope so styled-components can
// deduplicate them across every render.  We guard their APPLICATION
// (not their declaration) on prefers-reduced-motion at the component
// level below.
const bugPulseKf = keyframes`
  0%, 100% { opacity: 0.72; }
  50%      { opacity: 0.92; }
`;
const upNextSlideKf = keyframes`
  0%   { opacity: 0; transform: translateX(-12px); }
  100% { opacity: 1; transform: translateX(0); }
`;
const stationIdFadeKf = keyframes`
  0%   { opacity: 0; }
  8%   { opacity: 1; }
  70%  { opacity: 1; }
  100% { opacity: 0; }
...
/** Persistent channel bug — the faded "MTV" in the corner. */
const ChannelBug = styled.div<{ $reducedMotion: boolean }>`
  position: absolute;
  top: 14px;
  right: 18px;
  z-index: 8;
  ...
  border: 1px solid rgba(245, 233, 198, 0.18);
  border-radius: 2px;
  backdrop-filter: blur(1px);
  text-shadow: 0 0 6px rgba(245, 233, 198, 0.35);
  opacity: 0.82;

  /* Faint "breathing" glow only if the viewer allows motion. */
  ${({ $reducedMotion }) =>
    $reducedMotion
      ? css``
      : css`
          animation: ${bugPulseKf} 6s ease-in-out infinite;
        `}
`;

/** Lower-third "Up Next" card that slides up during bumpers. */
const UpNextCard = styled.div<{ $reducedMotion: boolean }>`
  position: absolute;
  left: 24px;
  bottom: 28px;
  z-index: 9;
  ...
  background: linear-gradient(
    135deg,
    rgba(6, 8, 10, 0.78) 0%,
    rgba(20, 22, 28, 0.6) 100%
  );
  border-left: 3px solid #f5e9c6;
  box-shadow:
    0 4px 20px rgba(0, 0, 0, 0.45),
    0 0 16px rgba(245, 233, 198, 0.08);
  color: #f5e9c6;
  font-family: "VT323", "IBM Plex Mono", monospace;
  ...
  ${({ $reducedMotion }) =>
    $reducedMotion
      ? css`
        ...
        `
      : css`
          animation: ${upNextSlideKf} 420ms cubic-bezier(0.22, 1, 0.36, 1) both;
        `}
  ...
  .title {
    ...
    color: #fff6d4;
    text-shadow: 0 0 8px rgba(245, 233, 198, 0.18);
    /* Clamp long titles to 2 lines so a pathological entry can't
       overrun the safe area of the CRT.  The webkit line-clamp idiom
       is the universally-supported way to do this today. */
    display: -webkit-box;
    -webkit-line-clamp: 2;
  ...
  .creator {
    font-size: 14px;
    letter-spacing: 0.06em;
    color: rgba(245, 233, 198, 0.78);
    margin-top: 4px;
  }
`;

/** Brief station-ID card that flashes on channel change. */
const StationIdCard = styled.div<{ $reducedMotion: boolean }>`
  position: absolute;
  inset: 0;
  z-index: 11;
  pointer-events: none;
  ...
  ${({ $reducedMotion }) =>
    $reducedMotion
      ? css`
        ...
        `
      : css`
          animation: ${stationIdFadeKf} 2600ms ease-out forwards;
        `}
  ...
  .frame {
    ...
    box-shadow:
      0 0 28px rgba(245, 233, 198, 0.08),
      inset 0 0 40px rgba(245, 233, 198, 0.05);
    font-family: "VT323", "IBM Plex Mono", monospace;
    color: #f5e9c6;
    text-align: center;
  }
  ...
  .big {
    ...
    line-height: 1;
    text-transform: uppercase;
    text-shadow: 0 0 14px rgba(245, 233, 198, 0.35);
  }
  .sub {
    font-size: 14px;
    letter-spacing: 0.18em;
    text-transform: uppercase;
    color: rgba(245, 233, 198, 0.8);
  }
`;
...
/**
 * Post a per-item lifecycle event to the server telemetry ring.
 *
 * Fire-and-forget: we prefer `navigator.sendBeacon` so the report
 * survives page unloads (tab close, channel navigation, route swap).
 * Failures are intentionally swallowed — the server side is an
 * advisory signal only, nothing depends on the POST succeeding.
 *
 * The server aggregates by `videoId` / `bumperId` (see
 * `server/routes/tv.ts → recordTelemetry`); after N distinct
 * sessions report an error on the same id within the rolling window
 * it silently drops from future queues.  That's how WTF TV self-heals
 * from a broken clip — the first viewer's report nudges it toward
 * removal, the third seals the deal.
 */
...
export function TV2() {
  ...
  // One id per TV2 mount — gives the server-side telemetry ring a way
  // to count "distinct sessions" without needing a cookie or a login.
  // Survives navigation inside the app (refs don't reset on rerender)
  // but resets on full reload, which matches the audit's notion of a
  // viewer session.
  const sessionIdRef = useRef<string>(
    typeof crypto !== "undefined" && typeof crypto.randomUUID === "function"
      ? crypto.randomUUID()
      : `tv2-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`
  );
  ...
  // Muted state is orthogonal to volume — hitting M should toggle
  // mute without destroying the volume setpoint, so when the viewer
  // un-mutes they hear the same level they had before.
  ...
  /**
   * Respect `prefers-reduced-motion`.  When set, the TV2 chrome
   * drops the static flicker, pulse animations, slide transitions,
   * and the station-ID fade — we still show the same *information*,
   * it just doesn't move.  This is one of the two accessibility
   * items the audit flagged and is cheap to honour.
   *
   * We subscribe via MediaQueryList so a viewer toggling the OS
   * setting mid-session updates in real time.
   */
  ...
  /**
   * Station-ID card shown briefly on every channel change.  Nothing
   * fancy — we flip it on when the channel id changes, auto-clear
   * after ~2.6 s (matches the CSS fade).  Motion-reduced users still
   * see it, but without the fade animation it simply holds for the
   * same duration then disappears.
   */
  const [showStationId, setShowStationId] = useState(false);
  const stationIdTimerRef = useRef<number | null>(null);
  useEffect(() => {
    if (!powerOn || !selectedChannelId) {
      setShowStationId(false);
      return;
    }
    setShowStationId(true);
    if (stationIdTimerRef.current) {
      window.clearTimeout(stationIdTimerRef.current);
    }
    stationIdTimerRef.current = window.setTimeout(() => {
      setShowStationId(false);
  ...
  // TV2 skip-notice UX.  When a clip errors we flash a short banner so
  // the viewer knows the channel is still moving.  Cleared on an
  // auto-timer rather than tied to the next item starting — the
  // bumper/queue-advance path is async and the banner would otherwise
  // hang around for a surprising length of time.
  const [skipNotice, setSkipNotice] = useState<string | null>(null);
  const skipNoticeTimerRef = useRef<number | null>(null);
  // Session-scoped per-item failure counter.  Keys are "itemId" /
  // "bumper:id" strings so we don't collide video items with bumpers.
  // Threshold is 2: one transient error earns a retry (common with
  // flaky IPFS gateways), two in a row marks the clip as broken for
  // the rest of the session so the queue naturally walks past it.
  ...
  /**
   * The next non-bumper item after the current cursor, used for the
   * "Up Next" lower-third card.  We walk forward (wrapping once)
   * until we find a real video; if the queue is all bumpers or
   * empty, this ends up null and the Up-Next card doesn't render.
   *
   * We intentionally ignore bumpers for the label — the card is
   * supposed to tease what's *coming from the playlist*, not flash
   * the next interstitial graphic.
   */
  const upNextItem = useMemo<StreamQueueItem | null>(() => {
    const queue = queueItems;
    const len = queue.length;
    if (len === 0) return null;
    for (let i = 1; i <= len; i++) {
      const idx = (clientQueueIdx + i) % len;
      const item = queue[idx];
      if (!item) continue;
      if (item.kind !== "bumper") return item;
    }
    return null;
  ...
  const flashSkipNotice = useCallback((message: string) => {
    ...
    // Match the CSS animation's natural end (~2.6s) so the DOM node
    // tears down after its fade-out rather than lingering invisibly.
  ...
  const handleCurrentMediaError = useCallback(() => {
    ...
    // Track how many times THIS item has errored during the session.
    // Two strikes and it lands on the skip-list: the queue-advance
    // paths below check `sessionSkipListRef` so future loops won't
    // keep slamming into the same broken file.
    ...
    const justBlacklisted = nextFails >= 2 && !sessionSkipListRef.current.has(failKey);
    if (justBlacklisted) {
    ...
    // Dispatch to the server telemetry ring so other sessions
    // benefit from this failure too.  See `reportItemEnd` comment for
    // how this feeds the server-side blacklist.
    ...
    // Give the viewer a one-line readable signal instead of the
    // old silent-freeze → atmospheric-static behaviour.  Copy is
    // intentionally plain English so it feels like a broadcast
    // engineer voice, not a React error boundary.
    flashSkipNotice(
      justBlacklisted
  ...
  /** Channel DOWN helper — mirror of `cycleChannel` for the remote. */
  const cycleChannelDown = useCallback(() => {
    if (channels.length === 0) return;
    if (!selectedChannelId) {
      setSelectedChannelId(channels[0]!.id);
      return;
    ...
    const idx = channels.findIndex((c) => c.id === selectedChannelId);
    const next = idx <= 0 ? channels.length - 1 : idx - 1;
  ...
  /**
   * Keyboard "remote control".  Bound to the window, active any time
   * TV2 is mounted.  Deliberately ignores events that originate from
   * form controls so the remote doesn't fight normal typing in
   * editable areas (CREATOR forms, playlist pickers, etc.).
   *
   * Mapping (matches the on-screen knob affordances):
   *   ↑ / ↓         — channel up / down
   *   Space         — toggle power
   *   M             — toggle mute
   *   + / =         — volume up (5%)
   *   - / _         — volume down (5%)
   *   0             — volume off (soft mute via setVolume)
   *
   * We only intercept when the event is a pure key press (no Ctrl,
   * Meta, or Alt modifier) to avoid breaking browser shortcuts.
   */
  useEffect(() => {
    if (typeof window === "undefined") return;
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case " ":
        case "Spacebar":
          // Only grab the spacebar if the TV surface is visible —
          // otherwise let the browser handle it (scroll, button
          // activation, etc.).  Power toggles from the OSD anyway.
          if (screenView !== "tv" && screenView !== "menu") return;
          e.preventDefault();
          setPowerOn((v) => !v);
          return;
        case "m":
  ...
  }, [powerOn, screenView, cycleChannel, cycleChannelDown]);
            ...
            <ScreenBay>
              <ScreenBezel>
                <CRTScreen $on={powerOn}>
                    ...
                    currentMediaUrl && (
                      <MediaVideo
                        ...
                        onEnded={(e) => {
                          ...
                          // Server-side telemetry: a natural end
                          // counts as a completion so future queues
                          // can favour clips that viewers actually
                          // sit through.  Bumpers end via
                          // `finishTransition`, not this handler, so
                          // we only pass `videoId`.
                  ...
                  {/* TV static cycles at ~24 Hz and is genuinely
                      rough on motion-sensitive viewers.  When the OS
                      requests reduced motion we replace it with a
                      solid deep-grey wash — same informational
                      purpose (we're between signals), no flicker. */}
                  {showStatic && screenView === "tv" && !prefersReducedMotion && (
                    <TVStatic audio={volume > 0.01 && !muted} />
                  )}
                  {showStatic && screenView === "tv" && prefersReducedMotion && (
                    <div
                      aria-hidden
                      style={{
                        ...
                        inset: 0,
                        background:
                          "radial-gradient(ellipse at center, #1a1c20 0%, #0b0c0f 80%)",
                        zIndex: 7,
                      }}
                    />
                  )}
                  ...
                  {/* TV2: transient skip-notice banner — see
                      SkipNoticeBanner declaration for rationale.  Rendered
                      inside the CRT screen, above the video/bumper stack
                      but below the chrome overlays, so it reads as part
                      of the broadcast rather than the app UI.  `aria-live`
                      makes screen readers announce the skip, addressing
                      the audit's accessibility finding about silent
                      failures. */}
                  ...
                  {/* MTV-style persistent channel bug — top-right,
                      faded, shows dial number + channel shortname.
                      Lives on the screen any time the TV is on and
                      the viewer is on the broadcast surface, exactly
                      like the real thing. */}
                  {powerOn &&
                    screenView === "tv" &&
                    !hasNoContent &&
                    currentChannel && (
                      ...
                      >
                        ...
                        <span style={{ opacity: 0.85 }}>
                          {(currentChannel.title || "WTF TV").slice(0, 18)}
                        </span>
                      </ChannelBug>
                    )}

                  {/* MTV-style "Up Next" lower-third.  Only visible
                      while a bumper is playing — the viewer already
                      knows what they're watching when a real clip is
                      on screen, but a bumper is the natural gap to
                      tease the next item.  Hidden when we don't have
                      a next item to tease (single-item playlist, or
                      queue is nothing but bumpers). */}
                  {powerOn &&
                    screenView === "tv" &&
                    showBumper &&
                    upNextItem && (
                      <UpNextCard
                        $reducedMotion={prefersReducedMotion}
                        role="status"
                        aria-label={`Up next: ${upNextItem.title || "Untitled"}`}
                      >
                        <div className="label">Up Next</div>
                        <div className="title">
                          {upNextItem.title || "Untitled"}
                        </div>
                        {(upNextItem.creatorName ||
                          upNextItem.collectionName) && (
                  ...
                  {/* Station-ID card — a brief broadcast intro that
                      flashes on every channel change, giving each tune
                      in a real identity hit before content rolls.  We
                      show the daypart label from the server too so
                      viewers get a free cue ("Prime Time", "Late
                      Night") about the programming block. */}
                  {powerOn &&
                    screenView === "tv" &&
                    showStationId &&
                    currentChannel && (
                      ...
                      >
                        <div className="frame">
                          <div className="eyebrow">You are watching</div>
                          <div className="big">
                            {(currentChannel.title || "WTF TV")
                              .toUpperCase()
                              .slice(0, 24)}
                          </div>
                          <div className="sub">
                            CH {dialDisplay}
                            {streamQuery.data?.daypart?.displayName
                              ? " • " + streamQuery.data.daypart.displayName
                              : ""}
                          </div>
                        </div>
