...
Production publication helper: `npm run pasta:wtfme:live-publish` performs a dry-run with Pasta-specific credentials and prints the resolved user-site host; it only mutates production when `PASTA_WTFME_LIVE_PUBLISH=1` is set. `npm run pasta:wtfme:live-publish:check` locks the dry-run default, Pasta-scoped credentials, expected-host guard, CSRF writes, eligibility diagnostics, and post-publish TLS probe in source-policy coverage. `npm run pasta:wtfme:live-inventory` is the read-only production inventory probe for `/api/wtf-sites/my`, admin site listing, pin registry state, and TLS ask status, with `npm run pasta:wtfme:live-inventory:check` guarding that it does not call claim/save/publish APIs. A 2026-07-01 all-puppet production inventory run authenticated all 12 available puppet users, found zero existing WTF.ME sites and zero claimable hosts, and confirmed the common blockers are invalid DNS-label usernames plus missing OAuth social and Bluesky/active WTF DID state. The same run found three admin-readable users can list only `cobwebsaints.wtfos.me` as draft/TLS-allowed and `paulwhoisaghost.wtfos.me` as published/TLS-allowed, while `wtf-admin.wtfos.me` remains denied as `handle not registered`.
...
| WTF.ME hosted Pasta pages | `npx tsx --test server/features/wtf-sites/pasta-hosting.test.ts` validates immutable landing/mint/collection page snapshots and manifest digest behavior; `npm run pasta:shadownet:wtfme` claims the user site, saves the Pasta pages, publishes them through the harness WTF.ME APIs, then serves them from `wtf-admin.wtfos.me` with Shadownet proof KT1s, wallet/purchase markers, CSP/opener headers, and `wtf_site.claimed` / `wtf_site.page_saved` / `wtf_site.published` / `wtf_site.public.viewed` events. `npm run pasta:wtfme:live-publish` now provides a gated production API-publish helper, `npm run pasta:wtfme:live-publish:check` proves the helper remains dry-run/CSRF/host-guarded and reports eligibility blockers, `npm run pasta:wtfme:live-inventory` safely inventories eligible/current/admin hosts and TLS ask status, and `npm run pasta:wtfme:live-check` is the production host gate. The live gate currently fails because the production TLS ask gate denies `wtf-admin.wtfos.me` as `handle not registered`; all 12 available production puppets authenticate but expose no current site or claimable host because their handles are not DNS-valid and lack the required social/DID state. The read-only inventory probe found `cobwebsaints.wtfos.me` and `paulwhoisaghost.wtfos.me` TLS-allowed, but this is not Pasta publication proof without owner credentials or an intentional publish to an eligible host. Production live Pasta content, browser UI authoring persistence, hosted pinning/recovery, and wallet-signed purchase completion are still open. | PARTIAL |
| wtfOS Pasta pinning/recovery | `npm run pasta:shadownet:pinning` builds app.wtfos.media pinPolicy, pinManifest, and pinItem records from real Pasta hosted-page snapshots and contract artifacts, validates them against the AT lexicon, proves public storage refs, IPFS gateway URLs, object-storage mirror keys, `.well-known/wtfos-pins`, restore order, and a public-discovery recovery drill for hosted pages plus Pasta metadata, and asserts the public `/.well-known/wtfos-pins` route is not ready until a repo DID and published manifest AT URI are present. Live provider pin completion, published PDS records, object-store writes, and live persisted recovery drill are still open. | PARTIAL |
...
1. Run `npm run pasta:wtfme:live-inventory` with the Pasta-scoped or admin-scoped credentials being considered, confirm the candidate host is registered/TLS-allowed, then provision or identify a Pasta-specific production user whose username is a DNS-valid WTF.ME label and whose account has wallet plus OAuth social, linked Bluesky, or active WTF DID state.
