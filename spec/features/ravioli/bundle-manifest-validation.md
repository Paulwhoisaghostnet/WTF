# Ravioli: Bundle manifest validation

## Purpose

Bundle manifest validation lets Pasta Protocol users move the Ravioli workflow toward a valid token product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for bundle manifest validation.
...
- Validated state for bundle manifest validation.
...
- Standalone validation: open Ravioli, exercise bundle manifest validation, assert visible status and no console-modal dependency.
