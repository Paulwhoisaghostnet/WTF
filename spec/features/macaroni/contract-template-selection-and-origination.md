# Macaroni: Contract template selection and origination

## Purpose

Contract template selection and origination lets Pasta Protocol users move the Macaroni workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- The user can access `/tools/macaroni` and the relevant app gate is enabled.
...
- User-entered configuration for contract template selection and origination.
- Wallet/network context when the feature signs or reads chain state.
- Package, media, metadata, contract, or relationship data appropriate to Macaroni.

## Outputs

- Validated state for contract template selection and origination.
...
- Standalone validation: open Macaroni, exercise contract template selection and origination, assert visible status and no console-modal dependency.
