...
const INSTALLER_PLATFORMS = [
  {
    key: "macos",
    label: "macOS",
    env: "SPAGHETTI_INSTALLER_MACOS_URL",
    sha256Env: "SPAGHETTI_INSTALLER_MACOS_SHA256",
    fileName: "Spaghetti-Studio.dmg",
  },
  {
    key: "windows",
    label: "Windows",
    env: "SPAGHETTI_INSTALLER_WINDOWS_URL",
    sha256Env: "SPAGHETTI_INSTALLER_WINDOWS_SHA256",
    fileName: "Spaghetti-Studio.exe",
  },
  {
    key: "raspberry-pi",
    label: "Raspberry Pi",
    env: "SPAGHETTI_INSTALLER_RASPBERRY_PI_URL",
    sha256Env: "SPAGHETTI_INSTALLER_RASPBERRY_PI_SHA256",
    fileName: "spaghetti-studio-arm64.deb",
  },
] as const;
...
router.get("/api/spaghetti/installers", isAuthenticated, (_req, res) => {
  const version = String(process.env.SPAGHETTI_INSTALLER_VERSION || "").trim();
  return res.json({
    ok: true,
    product: "spaghetti",
