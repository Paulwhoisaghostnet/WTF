...
const EXPECTED_VERSION = process.env.ROTINI_EXPECTED_INSTALLER_VERSION || desktopPackage.version;
const EXPECTED_RELEASE_TAG = process.env.ROTINI_EXPECTED_RELEASE_TAG || `rotini-desktop-v${EXPECTED_VERSION}`;
const REPOSITORY = process.env.ROTINI_INSTALLER_REPOSITORY || DEFAULT_REPOSITORY;

const EXPECTED_ASSETS = {
  macos: {
    label: "macOS",
    manifestFileName: "Rotini-Studio.dmg",
    assetName: `Rotini-Studio-${EXPECTED_VERSION}-mac-universal.dmg`,
  },
  windows: {
    label: "Windows",
    manifestFileName: "Rotini-Studio.exe",
    assetName: `Rotini-Studio-${EXPECTED_VERSION}-win-x64.exe`,
  },
  "raspberry-pi": {
    label: "Raspberry Pi",
    manifestFileName: "rotini-studio-arm64.deb",
    assetName: `Rotini-Studio-${EXPECTED_VERSION}-linux-arm64.deb`,
  },
};

const failures = [];
const cookieJar = new Map();
const suppliedCookie = String(process.env.ROTINI_INSTALLER_COOKIE || process.env.WTFOS_INSTALLER_COOKIE || "").trim();
...
const checkAssets = flag("ROTINI_INSTALLER_CHECK_ASSETS", flag("WTFOS_INSTALLER_CHECK_ASSETS", true));
const requireAuth = flag("ROTINI_INSTALLER_REQUIRE_AUTH", flag("WTFOS_INSTALLER_REQUIRE_AUTH", false));
const username = String(process.env.ROTINI_INSTALLER_USERNAME || process.env.WTFOS_INSTALLER_USERNAME || "").trim();
const password = String(process.env.ROTINI_INSTALLER_PASSWORD || process.env.WTFOS_INSTALLER_PASSWORD || "");
...
function ok(message) {
  console.log(`[rotini-installers] ok: ${message}`);
}

function baseUrl() {
  const raw = String(
    process.env.ROTINI_INSTALLER_BASE_URL || process.env.WTFOS_INSTALLER_BASE_URL || process.env.WTFOS_BASE_URL || DEFAULT_BASE_URL
  ).trim();
...
async function loginIfConfigured() {
  ...
  if (!username && !password) {
    if (requireAuth) {
      fail("ROTINI_INSTALLER_REQUIRE_AUTH=1 but no cookie or username/password was provided");
    }
    return false;
  }
  if (!username || !password) {
    fail("both ROTINI_INSTALLER_USERNAME and ROTINI_INSTALLER_PASSWORD are required for password login");
    return false;
  }
...
async function fetchReleaseMetadata() {
  ...
  const headers = {
    ...
    "user-agent": "wtfos-rotini-installer-check",
  };
  ...
  if (Object.keys(assets).length === Object.keys(EXPECTED_ASSETS).length) {
    ok(`GitHub release ${EXPECTED_RELEASE_TAG} exposes expected Rotini assets`);
  }
  return assets;
}
...
function validateManifest(manifest, releaseAssets) {
  ...
  if (manifest.product !== "rotini") {
    fail(`installer manifest product mismatch: expected rotini, got ${manifest.product}`);
  }
  ...
  ok(`authenticated manifest matches Rotini Studio ${EXPECTED_VERSION}`);
}

async function assertUnauthenticatedManifestIsProtected() {
  const response = await fetch(new URL("/api/rotini/installers", baseUrl()), { redirect: "manual" });
...
async function main() {
  console.log(`[rotini-installers] checking ${baseUrl().origin} against ${EXPECTED_RELEASE_TAG}`);
  ...
  if (authenticated && releaseAssets) {
    const response = await fetchWithCookies("/api/rotini/installers");
    if (!response.ok) {
    ...
    console.log("[rotini-installers] skipped authenticated manifest check; set ROTINI_INSTALLER_COOKIE or username/password to enable it");
  }

  if (failures.length) {
    console.error("[rotini-installers] failed:");
    for (const failure of failures) console.error(`- ${failure}`);
    process.exit(1);
  }
  console.log("[rotini-installers] live installer readiness checks passed");
}

main().catch((error) => {
  console.error(`[rotini-installers] ${error.stack || error.message}`);
  process.exit(1);
});
