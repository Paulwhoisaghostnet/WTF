...
Single-token package import lets Pasta Protocol users move the Gnocchi workflow toward a valid token product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for single-token package import.
...
- Validated state for single-token package import.
...
- Standalone validation: open Gnocchi, exercise single-token package import, assert visible status and no console-modal dependency.
