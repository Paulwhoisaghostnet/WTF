# Lasagna: Set current revision

## Purpose

Set current revision lets Pasta Protocol users move the Lasagna workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for set current revision.
...
- Validated state for set current revision.
...
- Standalone validation: open Lasagna, exercise set current revision, assert visible status and no console-modal dependency.
