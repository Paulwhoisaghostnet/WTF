...
| Social | `/messages`, `/messages/dms/:id`, `/mail`, `/digest`, `/aim`, `/dear-diary`, `/messageboard`, `/w`, `/w/post/:id`, `/w/chat`, `/w/groupchat/:id`, `/chat`, `/chat/:id`, `/dicksword`, `/i-hate-telegram`, `/profile`, `/wtf-subdomains`, `/user/:username`. |
...
| AIM | Session | Open a buddy-list view over existing DM conversations; read DM-backed chat logs; send through the existing DM route without creating a parallel message backend. | `aim.chat.opened`, `aim.message.sent`, `dm.message.sent` |
