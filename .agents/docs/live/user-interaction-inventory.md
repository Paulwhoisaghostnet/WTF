...
| WTF OS admin surfaces | Admin | View every registered sub-app/tool/desktop item by domain and subdomain; open the app, open related central admin routes/tabs, adjust app gate visibility, inspect native settings/automation handles, and receive the disabled-by-admin runtime failure when a gated app is reached directly. | `admin.os_surface.viewed`, `admin.os_surface.app_opened`, `admin.os_surface.admin_route_opened`, `admin.app_gate.updated`, `desktop.app.disabled_by_admin` |
...
| Roles/permissions/app gates | Admin | View/toggle role permissions; reset permission set; view/update desktop and Start Menu app gates, enforce disabled app runtime denial, and keep gate-aligned MCP feature access. | `admin.permissions.updated`, `admin.permissions.reset`, `admin.app_gate.updated`, `desktop.app.disabled_by_admin` |
