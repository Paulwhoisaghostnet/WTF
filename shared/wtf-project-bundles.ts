...
export const WTF_PROJECT_BUNDLE_SECTIONS: readonly WtfProjectBundleSection[] = [
  ...
  },
  {
    ...
    route: "/agent",
    owner: "Agent",
    purpose: "AI coding workspace snapshots, project memory, provider adapter metadata, MCP grants, chat history, native git staging, and file state.",
    requiredArtifacts: ["agent-workspace.json", "memory.json", "files.json", "plan.json", "actions.json", "git.json", "provider-adapters.json", "permissions.json"],
    eventHandles: [
      "agent.plan.updated",
  ...
  {
    key: "boardStory",
    label: "Board / Story Assets",
    dwelling: "shared",
    route: "/messageboard",
    owner: "Gameshow",
    purpose: "Story posts, board attachments, moderation state, pinned context, and public/shared narrative assets.",
    requiredArtifacts: ["board-assets.json", "story-posts.json", "attachments.json", "moderation.json"],
    eventHandles: ["board.message.created", "board.attachment.added", "board.moderation.updated"],
  },
  {
    key: "challenges",
    label: "Challenges",
  ...
  },
  {
    ...
    route: "/terminal",
    owner: "Shell",
    purpose: "User-visible system events, exportable logs, verification output, incident trails, and support bundles.",
    requiredArtifacts: ["system-events.json", "verification.log", "incident-report.json", "support-bundle.json"],
    eventHandles: ["system.event.logged", "logs.exported", "incident.report.created"],
  },
  {
    key: "exports",
    label: "Exports",
...
export function buildWtfProjectBundleManifest(): WtfProjectBundleManifest {
  return {
    ...
    guarantees: [
      "Every creator work has a bundle root before it crosses Studio, Game Studio, TV, gallery, board/story, rewards, contracts, logs, exports, IPFS, or release surfaces.",
