...
The newest polish pass moves beta another step away from "Wikipedia with a skin." The first screen now uses a larger live headline, reduced live-rail copy, a smoother shell status bar, and a full-width live desktop band instead of a floating explanatory block. The launcher labels are short product controls, the people panel says "People are here" instead of describing the proof system, and the Admin Lab remains the intentional place for maps, runbooks, and evidence.

The latest user critique was correct: a beta shell can have accurate route maps and still fail if it feels like Wikipedia. The current pass changes the default surface from "read the ecosystem" to "enter the OS": an active quest window, live people/object/return doors, a compact session HUD, a visible NOW state on the current quest stage, a closed Admin Lab for evidence, and a visible next workspace on desktop. On mobile, the live rail now appears directly after the active quest instead of below the rail copy. This keeps the reports available without letting them define the product.
