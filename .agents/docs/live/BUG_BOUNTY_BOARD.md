...
| WTF-BB-332 | Verified | Codex Pasta cleanup/status continuation | 2026-07-01 | Repo hygiene / Pasta stale worktrees | P2 | 9 | 12 | 1 | 3 | 2 | Stale `WTF-pasta-deploy` checkout is gone; current cleanup audit keeps the regression warning, marks `codex/pasta-live-readiness` and `wip/full-working-tree-20260617` as historical material unsafe to replay, confirms Spaghetti/Macaroni/IPFS/catalog branches are promoted ancestors, tracks the current branch plus `origin/<current-branch>` as valid ongoing work, and now runs inside `pasta:live-readiness` by default |
...
- Evidence:
  ...
  - 2026-07-01 branch recheck: `codex/spaghetti-installer-live`, the Macaroni branches, and `codex/ipfs-pinning-organ` are ancestors of `origin/main`; `codex/pasta-live-readiness` is `39` current-main commits behind and `22` historical commits ahead, and `wip/full-working-tree-20260617` is `188` current-main commits behind and `2` old backup commits ahead.
  - Two-dot replay checks showed `origin/main..codex/pasta-live-readiness` would touch 118 files with 28,795 deletions, including deletion of newer installer apps/workflows, live-readiness gates, Colander action proof tooling, and proof reports; `origin/main..wip/full-working-tree-20260617` would touch 807 files with 194,914 deletions, including current installer apps, workflows, Pasta reports, spec files, and live readiness tooling.
  - 2026-07-01 cleanup gate pass: `npm run pasta:repo-cleanup:audit` returned `ok: true`, classified the active `codex/pasta-live-verification-docs` branch plus `origin/codex/pasta-live-verification-docs` as valid ongoing work, classified the merged `codex/pasta-readiness-catalog-live`, `codex/pasta-installer-audit-docs`, Spaghetti, Macaroni, and IPFS branches as promoted ancestors, classified `codex/pasta-live-readiness`/`origin/codex/pasta-live-readiness` as historical evidence unsafe to replay, and reported no unknown Pasta branches.
...
- Status: In Progress
- Owner/Session: Codex Pasta manifest payload verifier
- Score: C2 + F5 + S2 + P1(4) = 13
- Evidence:
