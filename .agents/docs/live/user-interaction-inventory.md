...
| Desktop OS | Desktop shell, Start Menu, taskbar, desktop icons, command palette, `/mission-control`, `/file-manager`, `/settings`, `/browser-boundaries`, `/terminal`, `/theme-builder`, `/notification-center`, `/desktop-settings`, desktop pet tray, desktop artifact layer, screen saver, custom cursor, MCP pairing. |
