# Colander: Post-operation state refresh

## Purpose

Post-operation state refresh lets Pasta Protocol users move the Colander workflow toward a valid management outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for post-operation state refresh.
...
- Validated state for post-operation state refresh.
...
- Standalone validation: open Colander, exercise post-operation state refresh, assert visible status and no console-modal dependency.
