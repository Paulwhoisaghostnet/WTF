# Rotini: Preview/render artifact export

## Purpose

Preview/render artifact export lets Pasta Protocol users move the Rotini workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for preview/render artifact export.
...
- Validated state for preview/render artifact export.
...
- Standalone validation: open Rotini, exercise preview/render artifact export, assert visible status and no console-modal dependency.
