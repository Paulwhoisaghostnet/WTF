# Rotini: Collection target selection

## Purpose

Collection target selection lets Pasta Protocol users move the Rotini workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for collection target selection.
...
- Validated state for collection target selection.
...
- Standalone validation: open Rotini, exercise collection target selection, assert visible status and no console-modal dependency.
