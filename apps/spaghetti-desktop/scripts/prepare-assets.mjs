...
const sourceDir = path.join(repoRoot, "public/creation-tools/spaghetti");
const iconSourceRoot = path.join(repoRoot, "public/pasta-icons");
const outDir = path.join(appDir, "spaghetti");
...
if (!existsSync(sourceDir)) {
  console.error(`Spaghetti source assets not found: ${sourceDir}`);
  process.exit(1);
}
...
for (const rel of required) {
  ...
  if (!existsSync(target) || statSync(target).size === 0) {
    console.error(`Missing Spaghetti desktop asset: ${rel}`);
    process.exit(1);
  }
}
...
console.log(`Prepared Spaghetti desktop assets in ${path.relative(repoRoot, outDir)}`);
