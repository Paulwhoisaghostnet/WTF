...
function rotiniRoot() {
  return path.join(appRoot(), "rotini");
}
...
function safeStaticPath(urlPath) {
  ...
  const fullPath = path.join(rotiniRoot(), rel);
  const root = rotiniRoot();
...
async function handleRequest(req, res) {
  ...
  if (req.method === "GET" && parsed.pathname === "/api/auth/user") {
    return sendJson(res, 401, {
      error: "Rotini Desktop does not include wtfOS hosted resources.",
      native: true,
      product: "rotini-studio",
    });
  }
  if (req.method === "GET" && parsed.pathname === "/api/auth/csrf-token") {
    return sendJson(res, 200, { csrfToken: "rotini-desktop" });
  }
  if (req.method === "GET" && parsed.pathname === "/api/rotini/installers") {
  ...
  if (
    ...
    return sendJson(res, 403, {
      error: "wtfOS hosted pinning, publishing, and preview processing are not available in Rotini Desktop. Use Pinata or your own IPFS node.",
      native: true,
      product: "rotini-studio",
    });
  }
