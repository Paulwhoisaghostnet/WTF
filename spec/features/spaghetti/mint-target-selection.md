# Spaghetti: Mint target selection

## Purpose

Mint target selection lets Pasta Protocol users move the Spaghetti workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for mint target selection.
...
- Validated state for mint target selection.
...
- Standalone validation: open Spaghetti, exercise mint target selection, assert visible status and no console-modal dependency.
