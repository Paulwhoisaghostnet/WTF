...
2. Publish the generated Pasta landing/mint/collection pages to a claimed production WTF.ME host and make `PASTA_WTFME_LIVE_HOST=<published-host> npm run pasta:wtfme:live-check` pass, including `.well-known/wtfos-pins`, DID resolution, and the public PDS `pinManifest` record.
