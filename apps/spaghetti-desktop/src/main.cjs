...
const PRODUCT_NAME = "Spaghetti Studio";
...
function spaghettiRoot() {
  return path.join(appRoot(), "spaghetti");
}
...
function safeStaticPath(urlPath) {
  ...
  const fullPath = path.join(spaghettiRoot(), rel);
  const root = spaghettiRoot();
...
async function handleRequest(req, res) {
  ...
  if (req.method === "GET" && parsed.pathname === "/api/auth/user") {
    return sendJson(res, 401, {
      error: "Spaghetti Desktop does not include wtfOS hosted resources.",
      native: true,
      product: "spaghetti-studio",
    });
  }
  if (req.method === "GET" && parsed.pathname === "/api/auth/csrf-token") {
    return sendJson(res, 200, { csrfToken: "spaghetti-desktop" });
  }
  if (req.method === "GET" && parsed.pathname === "/api/spaghetti/installers") {
  ...
  if (
    ...
    return sendJson(res, 403, {
      error: "wtfOS hosted pinning, publishing, and preview processing are not available in Spaghetti Desktop. Use Pinata or your own IPFS node.",
      native: true,
      product: "spaghetti-studio",
    });
  }
...
if (!hasSingleInstanceLock) {
  ...
  app.whenReady().then(async () => {
    try {
      ...
      dialog.showErrorBox("Spaghetti Studio failed to start", err.message || String(err));
      app.quit();
    }
