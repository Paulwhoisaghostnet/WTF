...
- Clean promotion branch `codex/spaghetti-installer-live` is currently the Pasta release lane and ahead of `origin/main` by 24 scoped commits after this pass: WTF.ME hosted-page proof, Pasta pinning/recovery proof, hardened pin recovery publish path, suite manifest `bundledApps`, explicit-host WTF.ME live check, explicit/discovered-host live inventory, stale-branch classification refresh, live publisher overwrite guards, live publisher expected-host pinning, a Pasta live-readiness gate, installer-aware readiness coverage, post-publish public host verification, credentialed publish dry-run validation, signer-backed Colander management action proof, Colander browser-wallet choreography proof, non-spending recorded Colander action verification inside the readiness gate, public manifest-payload verifier wiring, standalone installer source plumbing for Gnocchi, Ravioli, Rotini, Penne, and Lasagna, individual-installer readiness blockers, and a standalone installer release audit helper.
...
- Gnocchi Studio Desktop standalone installer package, GitHub Actions workflow, authenticated `/api/gnocchi/installers` manifest route, production env names, live verifier, docs, and inventory probe are source-verified for macOS, Windows, and Raspberry Pi; release assets, production env, and authenticated live verification are still pending.
- Ravioli, Rotini, Penne, and Lasagna Studio Desktop standalone installer packages, GitHub Actions workflows, authenticated installer manifest routes, production env names, live verifiers, docs, and inventory probes are source-verified for macOS, Windows, and Raspberry Pi; release assets, production env, and authenticated live verification are still pending.
- `npm run pasta:standalone-installers:audit` now provides the executable release checklist for those five standalone installers. On 2026-07-01 it passed local source/policy checks for all five apps and blocked, correctly, because the remote workflows are not registered on `origin/main`, the five `*-desktop-v1.0.0` release tags do not exist, and live `wtfos.app` still returns 404 for their manifest routes.
...
| `.config/superpowers/worktrees/WTF/codex-spaghetti-installer-live` | `codex/spaghetti-installer-live` | 0 | Current Pasta production/promotion checkout | Clean continuation lane, ahead of `origin/main` by 24 scoped commits after this pass. This is the only Pasta release branch to promote from; still blocked from a full production Pasta claim by live WTF.ME host and pin-discovery proof plus individual installer release publication for Gnocchi/Ravioli/Rotini/Penne/Lasagna. |
...
6. Promote the standalone installer workflow/route commits, then run `PASTA_STANDALONE_INSTALLER_AUDIT_ALLOW_BLOCKERS=1 npm run pasta:standalone-installers:audit` to track each remaining Gnocchi/Ravioli/Rotini/Penne/Lasagna release step: remote workflow registration, release asset publication, production env/deploy, and authenticated live verifier pass. Decide whether Colander should ship as a separate native admin app or remain an in-suite/main-app surface.
