...
| Desktop OS | Desktop shell, Start Menu, taskbar, desktop icons, command palette, `/mission-control`, `/file-manager`, `/settings`, `/browser-boundaries`, `/terminal`, `/desktop-settings`, desktop pet tray, desktop artifact layer, screen saver, custom cursor, MCP pairing. |
...
| Browser Boundaries | Session | Open `/browser-boundaries`; inspect the standard access manifest, public/session/role route split, public API count, MCP bearer-token boundary, CSRF write boundary, and CSP frame boundary without exposing private payloads or secrets. | `browser_boundaries.viewed`, `desktop.window.opened` |
...
| Notification Center | Session | Open `/notifications` as a first-class OS surface; view all/unread changes, adjust notification preferences, open supported linked targets, and mark one/all notifications read without entering direct-message threads first. | `notification.viewed`, `notification.opened`, `notification.read`, `notification.read_all`, `notification.preference.updated` |
| Recovery Mode | Session | View user-safe incident state for health, media cache, wallet sessions, chain network overrides, and saved shell layout; disconnect local Tezos/Etherlink wallet sessions; reset local chain network overrides; clear saved window session; export a local recovery report; route operator-only permissions reset, app rollback, and restore-proof work to admin-gated surfaces. | `recovery_mode.viewed`, `recovery_mode.wallets_disconnected`, `recovery_mode.network_reset`, `recovery_mode.window_session_cleared`, `recovery_mode.report_exported` |
...
| W timeline | Session/staff | View filtered-stream-backed cached/live timeline; refresh; compose post; upload media; reply; like; repost; quote; open post detail; view diagnostics/source/staleness; use link preview; staff manage packed stream rules and the admin manifest of extra timeline handles. | `w.timeline.viewed`, `w.post.created`, `w.media.uploaded`, `w.reply.created`, `w.like.created`, `w.repost.created`, `w.quote.created`, `w.link_preview.requested`, `w.admin.stream_rule.updated`, `w.admin.stream_manifest.updated` |
| W groupchat | Session/staff | View only configured official Gameshow groupchats; send explicit groupchat messages with the acting user's X authorization; staff manage official groupchat IDs and XAA-backed groupchat wakeups. Personal X DM inboxes and ad hoc user DM threads are not an active W surface. | `w.groupchat.viewed`, `w.groupchat.message_sent`, `w.admin.groupchat.updated`, `w.xaa.groupchat_wakeup.received` |
...
| Social spam | Board/DM/W post/reaction bursts, slow-mode hits, webhook abuse, media attachment spam, repeated deleted/moderated posts. | `board.message.created`, `messageboard.reaction.added`, `messageboard.reaction.removed`, `dm.message.sent`, `w.post.created`, `board.webhook_received`, `system.rate_limit.hit` |
