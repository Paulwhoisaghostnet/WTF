...
- Spaghetti Studio Desktop `1.0.0` standalone installer package, GitHub Actions workflow, authenticated `/api/spaghetti/installers` manifest route, production env names, live verifier, docs, and inventory probe exist on the proof branch; release assets and production env/live verification are not yet published.
...
- Production WTF.ME API publication now has an explicit helper: `npm run pasta:wtfme:live-publish` dry-runs the authenticated user's eligible `username.wtfos.me` host, reports missing eligibility requirements, and only with `PASTA_WTFME_LIVE_PUBLISH=1` claims, saves, and publishes the Pasta pages through the normal WTF.ME APIs.
- Production WTF.ME inventory now has a read-only helper: `npm run pasta:wtfme:live-inventory` authenticates with Pasta-scoped credentials, reads `/api/wtf-sites/my`, reads `/api/admin/wtf-sites` when permitted, summarizes pin registry state, and probes `/internal/tls/allow` for candidate hosts without calling claim/save/publish APIs.
- Production credential discovery on 2026-07-01 found all 12 available puppet users authenticate but none provides a Pasta publish identity: every puppet has a wallet, but every username is not a valid DNS label, none has OAuth social, none has linked Bluesky or an active WTF DID, none has an existing WTF.ME site, and none exposes a claimable host. Three elevated puppets can read the admin site list, which contains only `cobwebsaints.wtfos.me` as draft/TLS-allowed and `paulwhoisaghost.wtfos.me` as published/TLS-allowed; `wtf-admin.wtfos.me` remains denied as `handle not registered`.
- wtfOS Pasta pinning/recovery record proof now builds app.wtfos.media pinPolicy, pinManifest, and pinItem records from the Pasta hosted pages and real static contract artifacts, validates them against the AT lexicon, proves public storage refs, IPFS gateway URLs, object-storage mirror keys, `.well-known/wtfos-pins`, and restore order for hosted pages plus Pasta metadata, and keeps the public discovery route closed until the binding has both a repo DID and a published manifest AT URI.
...
- Spaghetti standalone installer/download is branch-complete but not production-complete: `apps/spaghetti-desktop`, the `spaghetti-desktop-installers.yml` workflow, `/api/spaghetti/installers`, inventory coverage, package policy checks, env examples, docs, and live verifier now exist, and the local macOS unpacked app build passed; a `spaghetti-desktop-v1.0.0` GitHub release, production `SPAGHETTI_INSTALLER_*` env, container reload, and authenticated live verifier are still required before users can download it from `wtfos.app`.
- Full Pasta contract/product workflow proof remains broader than static bundle availability, the now-green Macaroni Shadownet confidence lane, the Spaghetti Shadownet preflight, the signer-backed Spaghetti/Gnocchi/Ravioli/Rotini/Penne/Lasagna deploy/mint/collect/distribution/exhibition proofs, Colander discovery, local WTF.ME API-publish hosted-page proof, and wtfOS pin-record proof: failure recovery, production WTF.ME publish/live TLS/content proof, signed hosted-page purchase completion, live provider/PDS/object-store pinning, and browser-wallet Colander mutation/post-operation refresh should remain open until executable evidence exists. The exact app-by-app gate state is recorded in `.agents/docs/live/PASTA_LIVE_READINESS_MATRIX.md`.
...
- Live publish helper: `npm run pasta:wtfme:live-publish` dry-runs production host eligibility and prints claim blockers; `PASTA_WTFME_LIVE_PUBLISH=1 npm run pasta:wtfme:live-publish` performs the claim/save/publish sequence when production publication is explicitly intended.
- Live publish safety command: `npm run pasta:wtfme:live-publish:check` proves the production helper stays dry-run by default, uses only Pasta-scoped credentials, enforces `PASTA_WTFME_LIVE_EXPECT_HOST`, sends CSRF headers on mutating API calls, reports WTF.ME eligibility blockers, and rechecks the TLS ask gate after publishing.
- Live inventory helper: `npm run pasta:wtfme:live-inventory` is read-only apart from login, reports the current user's site/eligibility, optional admin site list, pin registry binding, candidate TLS ask status, and next required host; `npm run pasta:wtfme:live-inventory:check` guards that it does not call claim/save/publish APIs.
...
- User-site proof: the harness response includes `X-WTFOS-Surface: user-site`, wallet-compatible Content Security Policy sources, `Cross-Origin-Opener-Policy: same-origin-allow-popups`, and `wtf_site.claimed`, `wtf_site.page_saved`, `wtf_site.published`, and `wtf_site.public.viewed` events for the claim/save/publish/public-view path.
- Scope note: this proves branch-level hosted page API publishing and rendering through the WTF.ME user-site host contract. It does not yet prove live production DNS/TLS, browser UI authoring persistence, hosted wtfOS pinning/recovery, or a signed hosted-page mint/purchase operation.
- Live blocker: `npm run pasta:wtfme:live-check` now verifies the same page markers, user-site headers, Shadownet proof contracts, and public pin discovery against the real production host. DNS resolves `wtf-admin.wtfos.me` to the WTF host, but `https://wtfos.app/internal/tls/allow?domain=wtf-admin.wtfos.me` returns `handle not registered`, so the live gate fails before content verification.
- Live publish dry-run evidence: a 2026-07-01 production dry-run with `e2e_cookiemonster` authenticated and failed closed with `claimableHost=none`, `canClaim=false`, `hasWallet=true`, `hasOAuthSocial=false`, `hasLinkedBluesky=false`, `hasActiveWtfDid=false`, and reasons for invalid DNS label plus missing OAuth/social/DID state. This means the next live publication step needs a dedicated DNS-valid Pasta user or existing owner credentials for a suitable production host.
- Live inventory evidence: a 2026-07-01 read-only sweep across all 12 production puppets returned `ok` for every login, `mySite=null` for every user, zero claimable hosts, and the same invalid DNS-label plus missing OAuth/social/DID reasons. Three elevated puppets could read the admin list, which returned only `cobwebsaints.wtfos.me` (`draft`, home page, TLS allowed) and `paulwhoisaghost.wtfos.me` (`published`, home plus `airporters-vol-1`, TLS allowed); `wtf-admin.wtfos.me` still returned TLS ask `403` / `handle not registered`.

## Current wtfOS Pasta Pinning/Recovery Proof
...
- Public route proof: `server/features/ipfs-pinning/well-known-policy.test.ts` asserts `/.well-known/wtfos-pins` is ready only when public discovery is enabled and the subdomain binding includes a valid repo DID plus matching `app.wtfos.media.pinManifest` AT URI; incomplete bindings stay 404 instead of returning a partial discovery body.
...
- Inventory proof added on branch: `spaghetti.installer_manifest.viewed` and `/api/spaghetti/installers` are registered in `tests/e2e/inventory/domain-workflows.mjs`; run `npm run test:e2e:inventory:coverage` before promoting.
- Scope note: this is branch release-readiness proof only. It does not yet prove a published GitHub release, production `SPAGHETTI_INSTALLER_*` env, reloaded production app container, or live downloadable assets on `https://wtfos.app`.

## Recommended Next Actions

1. Use `.agents/docs/live/PASTA_LIVE_READINESS_MATRIX.md` as the live-push checklist, keep `npm run pasta:wtfme:live-publish:check` and `npm run pasta:wtfme:live-inventory:check` green, inventory candidate production hosts with `npm run pasta:wtfme:live-inventory`, provision or identify a DNS-valid Pasta production user with the required wallet/social/DID state, then run `npm run pasta:wtfme:live-publish` in dry-run mode before any live mutation.
2. Intentionally publish with `PASTA_WTFME_LIVE_PUBLISH=1` only after the dry-run resolves the expected host, and make `PASTA_WTFME_LIVE_HOST=<published-host> npm run pasta:wtfme:live-check` pass before signed mint/purchase validation.
3. Publish the Spaghetti Desktop release assets with tag `spaghetti-desktop-v1.0.0`, configure production `SPAGHETTI_INSTALLER_*` env from GitHub release digests, reload the production container, then run `SPAGHETTI_INSTALLER_COOKIE='connect.sid=...' npm run spaghetti:installers:live-check`.
...
5. Promote wtfOS Pasta pinning from record-shape proof to live provider/PDS/object-store durability plus a recovery drill.
6. Extend the now-green Shadownet evidence into one end-to-end Pasta chain: CH-EASE package -> publisher -> Shadownet deploy/mint -> Colander discovery -> hosted page -> artifact resolution.
7. After user confirmation, archive/delete merged historical Macaroni branches and clean checked-out worktrees that are ancestors of `origin/main`.
8. Keep the removed `WTF-pasta-deploy` archive only as recovery evidence; do not mine it for implementation unless a future pass needs a specific historical note.
