...
const EXPECTED_VERSION = process.env.PENNE_EXPECTED_INSTALLER_VERSION || desktopPackage.version;
const EXPECTED_RELEASE_TAG = process.env.PENNE_EXPECTED_RELEASE_TAG || `penne-desktop-v${EXPECTED_VERSION}`;
const REPOSITORY = process.env.PENNE_INSTALLER_REPOSITORY || DEFAULT_REPOSITORY;

const EXPECTED_ASSETS = {
  macos: {
    label: "macOS",
    manifestFileName: "Penne-Studio.dmg",
    assetName: `Penne-Studio-${EXPECTED_VERSION}-mac-universal.dmg`,
  },
  windows: {
    label: "Windows",
    manifestFileName: "Penne-Studio.exe",
    assetName: `Penne-Studio-${EXPECTED_VERSION}-win-x64.exe`,
  },
  "raspberry-pi": {
    label: "Raspberry Pi",
    manifestFileName: "penne-studio-arm64.deb",
    assetName: `Penne-Studio-${EXPECTED_VERSION}-linux-arm64.deb`,
  },
};

const failures = [];
const cookieJar = new Map();
const suppliedCookie = String(process.env.PENNE_INSTALLER_COOKIE || process.env.WTFOS_INSTALLER_COOKIE || "").trim();
...
const checkAssets = flag("PENNE_INSTALLER_CHECK_ASSETS", flag("WTFOS_INSTALLER_CHECK_ASSETS", true));
const requireAuth = flag("PENNE_INSTALLER_REQUIRE_AUTH", flag("WTFOS_INSTALLER_REQUIRE_AUTH", false));
const username = String(process.env.PENNE_INSTALLER_USERNAME || process.env.WTFOS_INSTALLER_USERNAME || "").trim();
const password = String(process.env.PENNE_INSTALLER_PASSWORD || process.env.WTFOS_INSTALLER_PASSWORD || "");
...
function ok(message) {
  console.log(`[penne-installers] ok: ${message}`);
}

function baseUrl() {
  const raw = String(
    process.env.PENNE_INSTALLER_BASE_URL || process.env.WTFOS_INSTALLER_BASE_URL || process.env.WTFOS_BASE_URL || DEFAULT_BASE_URL
  ).trim();
...
async function loginIfConfigured() {
  ...
  if (!username && !password) {
    if (requireAuth) {
      fail("PENNE_INSTALLER_REQUIRE_AUTH=1 but no cookie or username/password was provided");
    }
    return false;
  }
  if (!username || !password) {
    fail("both PENNE_INSTALLER_USERNAME and PENNE_INSTALLER_PASSWORD are required for password login");
    return false;
  }
...
async function fetchReleaseMetadata() {
  ...
  const headers = {
    ...
    "user-agent": "wtfos-penne-installer-check",
  };
  ...
  if (Object.keys(assets).length === Object.keys(EXPECTED_ASSETS).length) {
    ok(`GitHub release ${EXPECTED_RELEASE_TAG} exposes expected Penne assets`);
  }
  return assets;
}
...
function validateManifest(manifest, releaseAssets) {
  ...
  if (manifest.product !== "penne") {
    fail(`installer manifest product mismatch: expected penne, got ${manifest.product}`);
  }
  ...
  ok(`authenticated manifest matches Penne Studio ${EXPECTED_VERSION}`);
}

async function assertUnauthenticatedManifestIsProtected() {
  const response = await fetch(new URL("/api/penne/installers", baseUrl()), { redirect: "manual" });
...
async function main() {
  console.log(`[penne-installers] checking ${baseUrl().origin} against ${EXPECTED_RELEASE_TAG}`);
  ...
  if (authenticated && releaseAssets) {
    const response = await fetchWithCookies("/api/penne/installers");
    if (!response.ok) {
    ...
    console.log("[penne-installers] skipped authenticated manifest check; set PENNE_INSTALLER_COOKIE or username/password to enable it");
  }

  if (failures.length) {
    console.error("[penne-installers] failed:");
    for (const failure of failures) console.error(`- ${failure}`);
    process.exit(1);
  }
  console.log("[penne-installers] live installer readiness checks passed");
}

main().catch((error) => {
  console.error(`[penne-installers] ${error.stack || error.message}`);
  process.exit(1);
});
