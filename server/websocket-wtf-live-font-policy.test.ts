...
test("WTF LIVE realtime chat sanitizer preserves the supported four-font pack and normalizes legacy aliases", () => {
  assert.match(
    websocketSource,
  ...
  assert.match(websocketSource, /"mek-mono": "terminal"/);
  assert.match(websocketSource, /"grout-display": "classic-95"/);
  assert.match(websocketSource, /mono: "terminal"/);
  assert.match(websocketSource, /serif: "serif-press"/);
  assert.match(websocketSource, /pixel: "classic-95"/);
  ...
  assert.match(harnessSource, /const LIVE_CHAT_FONTS = new Set\(\["wtfos-soft-system", "classic-95", "terminal", "serif-press"\]\);/);
  assert.match(harnessSource, /"mek-mono": "terminal"/);
  assert.match(harnessSource, /"grout-display": "classic-95"/);
