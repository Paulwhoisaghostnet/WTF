...
| Ravioli | Atomic wrapped-token pack router (escrowed, allocated-mint, generative-mint, hybrid) | Recipe commit/reveal, custody reservations, atomic open/burn, direct sale, roles/admin | Self-hosted buy/open page plus contents manifest | Router KT1 + typed Gnocchi/Rotini adapters + pack site artifact | Local SmartPy/compile proof passes all five modes, rollback, conservation, and PNG/GIF/ZIP output; fresh signer-backed five-mode Shadownet run remains the deployment gate |
...
- `PASTA_SHADOWNET_E2E_EXECUTE=1 npm run pasta:shadownet:ravioli:e2e` is the guarded signer-backed Ravioli five-mode proof. It originates the router, Gnocchi/Rotini collections, and both typed adapters, then proves escrow delivery, allocated mint, generative mint, hybrid fulfillment, direct sale, and TzKT operation-tree/indexing evidence. It has not been rerun for the current router revision in this local pass.
...
- The historical Ravioli Shadownet report for `KT1LF14kfDc3nGq8Vs26J2BykYixWeEfYqMQ` covers the retired manifest/redeem path. A current-router report must be written only after the guarded five-mode run proves all helper internal operations, wrapper conservation, generated output bindings, and TzKT indexing on fresh contracts.
