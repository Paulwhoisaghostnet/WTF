...
| Domain | Access | Possible interactions | Primary handles |
| --- | --- | --- | --- |
| Tezos wallets | Session | List linked wallets; passively rehydrate cached local wallet display state without requesting ownership proof; detect reverse and owned `.tez` domains for linked wallets; select a preferred Tezos domain identity; request challenge only from explicit connect/link or participation flows; link connected/manual address; unlink; set primary; sync profile tokens; sync one wallet; fetch wallet WTF balance/tokens/profile dossier. | `wallet.challenge.created`, `wallet.linked`, `wallet.unlinked`, `wallet.primary_set`, `wallet.tezos_domain_selected`, `wallet.sync_requested`, `wallet.balance.viewed` |
...
| Social spam | Board message bursts, W timeline action bursts, slow-mode hits, webhook abuse, media attachment spam, repeated deleted/moderated posts. | `board.message.created`, `messageboard.reaction.added`, `messageboard.reaction.removed`, `w.reply.created`, `w.like.created`, `w.repost.created`, `w.quote.created`, `board.webhook_received`, `system.rate_limit.hit` |
