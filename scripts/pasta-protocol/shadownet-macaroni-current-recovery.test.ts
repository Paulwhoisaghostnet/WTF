...
test("Macaroni restart authenticates only the exact two-event pre-write checkpoint", async () => {
  const checkpointRoot = path.join(
    ...
    "macaroni",
    "artifacts",
    "macaroni-current-recovery",
  );
  const [intentBytes, recoveredPrefixEventBytes, signersAuthenticatedEventBytes] = await Promise.all([
    readFile(path.join(checkpointRoot, "intent.json")),
    readFile(path.join(checkpointRoot, "events", "001-recovered-prefix-accepted.json")),
    readFile(path.join(checkpointRoot, "events", "002-signers-authenticated.json")),
  ]);
  ...
  assert.equal(authenticated.intent.checkpointId, MACARONI_CURRENT_RECOVERY_PREWRITE_CHECKPOINT.checkpointId);
...
test("Macaroni V1-submitted restart authenticates the complete 21-event chain", async () => {
  const checkpointRoot = path.join(
    root,
    "artifacts",
    "pasta-protocol-proof-runs",
  ...
  const eventDirectory = path.join(checkpointRoot, "events");
  const eventNames = (await readdir(eventDirectory)).sort();
  assert.equal(eventNames.length, 21);
  const [intentBytes, ...eventBytes] = await Promise.all([
    readFile(path.join(checkpointRoot, "intent.json")),
    ...eventNames.map((name) => readFile(path.join(eventDirectory, name))),
  ]);
  ...
  assert.equal(authenticated.events[6].payload.operationHash, "opL6Z2vJV1sFqozrZnhziL8T9uh6PfZjmiHNNpVe5eum7Kg2L2V");
  assert.equal(authenticated.events[20].payload.operationHash, "ons68f9ucj5uFfZKdmLdgA93c2RRmsyBkLLwGFBXppahbqyGQoV");
  assert.equal(authenticated.lastEventFileSha256, "56f4a1dcf2ce456d20dd7131ad431d279cc083f9bedf9198156d3e68b9ec503e");

  const drifted = eventBytes.map((bytes) => Buffer.from(bytes));
  drifted[20][drifted[20].length - 2] = drifted[20][drifted[20].length - 2] === 0x7d ? 0x20 : 0x7d;
  assert.throws(
    () => authenticateMacaroniV1SubmittedCheckpointDocuments({ intentBytes, eventBytes: drifted }),
