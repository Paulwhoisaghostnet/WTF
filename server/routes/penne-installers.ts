...
const INSTALLER_PLATFORMS = [
  {
    key: "macos",
    label: "macOS",
    env: "PENNE_INSTALLER_MACOS_URL",
    sha256Env: "PENNE_INSTALLER_MACOS_SHA256",
    fileName: "Penne-Studio.dmg",
  },
  {
    key: "windows",
    label: "Windows",
    env: "PENNE_INSTALLER_WINDOWS_URL",
    sha256Env: "PENNE_INSTALLER_WINDOWS_SHA256",
    fileName: "Penne-Studio.exe",
  },
  {
    key: "raspberry-pi",
    label: "Raspberry Pi",
    env: "PENNE_INSTALLER_RASPBERRY_PI_URL",
    sha256Env: "PENNE_INSTALLER_RASPBERRY_PI_SHA256",
    fileName: "penne-studio-arm64.deb",
  },
] as const;
...
router.get("/api/penne/installers", isAuthenticated, (_req, res) => {
  const version = String(process.env.PENNE_INSTALLER_VERSION || "").trim();
  return res.json({
    ok: true,
    product: "penne",
