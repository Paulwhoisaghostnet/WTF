# Ravioli: Bundle target selection

## Purpose

Bundle target selection lets Pasta Protocol users move the Ravioli workflow toward a valid token product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for bundle target selection.
...
- Validated state for bundle target selection.
...
- Standalone validation: open Ravioli, exercise bundle target selection, assert visible status and no console-modal dependency.
