...
const desktopPackage = JSON.parse(readFileSync("apps/ch-ease-desktop/package.json", "utf8"));
const EXPECTED_VERSION = process.env.CH_EASE_EXPECTED_INSTALLER_VERSION || desktopPackage.version;
const EXPECTED_RELEASE_TAG = process.env.CH_EASE_EXPECTED_RELEASE_TAG || `ch-ease-desktop-v${EXPECTED_VERSION}`;
const REPOSITORY = process.env.CH_EASE_INSTALLER_REPOSITORY || DEFAULT_REPOSITORY;

const EXPECTED_ASSETS = {
  macos: {
    label: "macOS",
    manifestFileName: "CH-EASE-Studio.dmg",
    assetName: `CH-EASE-Studio-${EXPECTED_VERSION}-mac-universal.dmg`,
  },
  windows: {
    label: "Windows",
    manifestFileName: "CH-EASE-Studio.exe",
    assetName: `CH-EASE-Studio-${EXPECTED_VERSION}-win-x64.exe`,
  },
  "raspberry-pi": {
    label: "Raspberry Pi",
    manifestFileName: "ch-ease-studio-arm64.deb",
    assetName: `CH-EASE-Studio-${EXPECTED_VERSION}-linux-arm64.deb`,
  },
};

const failures = [];
const cookieJar = new Map();
const suppliedCookie = String(process.env.CH_EASE_INSTALLER_COOKIE || process.env.WTFOS_INSTALLER_COOKIE || "").trim();
...
const checkAssets = flag("CH_EASE_INSTALLER_CHECK_ASSETS", flag("WTFOS_INSTALLER_CHECK_ASSETS", true));
const requireAuth = flag("CH_EASE_INSTALLER_REQUIRE_AUTH", flag("WTFOS_INSTALLER_REQUIRE_AUTH", false));
const username = String(process.env.CH_EASE_INSTALLER_USERNAME || process.env.WTFOS_INSTALLER_USERNAME || "").trim();
const password = String(process.env.CH_EASE_INSTALLER_PASSWORD || process.env.WTFOS_INSTALLER_PASSWORD || "");
...
function ok(message) {
  console.log(`[ch-ease-installers] ok: ${message}`);
}

function baseUrl() {
  const raw = String(
    process.env.CH_EASE_INSTALLER_BASE_URL || process.env.WTFOS_INSTALLER_BASE_URL || process.env.WTFOS_BASE_URL || DEFAULT_BASE_URL
  ).trim();
...
async function loginIfConfigured() {
  ...
  if (!username && !password) {
    if (requireAuth) {
      fail("CH_EASE_INSTALLER_REQUIRE_AUTH=1 but no cookie or username/password was provided");
    }
    return false;
  }
  if (!username || !password) {
    fail("both CH_EASE_INSTALLER_USERNAME and CH_EASE_INSTALLER_PASSWORD are required for password login");
    return false;
  }
...
async function fetchReleaseMetadata() {
  ...
  const headers = {
    ...
    "user-agent": "wtfos-ch-ease-installer-check",
  };
  ...
  if (Object.keys(assets).length === Object.keys(EXPECTED_ASSETS).length) {
    ok(`GitHub release ${EXPECTED_RELEASE_TAG} exposes expected CH-EASE assets`);
  }
  return assets;
}
...
function validateManifest(manifest, releaseAssets) {
  ...
  if (manifest.product !== "ch-ease") {
    fail(`installer manifest product mismatch: expected ch-ease, got ${manifest.product}`);
  }
  ...
  ok(`authenticated manifest matches CH-EASE Studio ${EXPECTED_VERSION}`);
}

async function assertUnauthenticatedManifestIsProtected() {
  const response = await fetch(new URL("/api/ch-ease/installers", baseUrl()), { redirect: "manual" });
...
async function main() {
  console.log(`[ch-ease-installers] checking ${baseUrl().origin} against ${EXPECTED_RELEASE_TAG}`);
  ...
  if (authenticated && releaseAssets) {
    const response = await fetchWithCookies("/api/ch-ease/installers");
    if (!response.ok) {
    ...
    console.log("[ch-ease-installers] skipped authenticated manifest check; set CH_EASE_INSTALLER_COOKIE or username/password to enable it");
  }

  if (failures.length) {
    console.error("[ch-ease-installers] failed:");
    for (const failure of failures) console.error(`- ${failure}`);
    process.exit(1);
  }
  console.log("[ch-ease-installers] live installer readiness checks passed");
}

main().catch((error) => {
  console.error(`[ch-ease-installers] ${error.stack || error.message}`);
  process.exit(1);
});
