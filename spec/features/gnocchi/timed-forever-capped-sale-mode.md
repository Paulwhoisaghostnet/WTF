# Gnocchi: Timed/forever/capped sale mode

## Purpose

Timed/forever/capped sale mode lets Pasta Protocol users move the Gnocchi workflow toward a valid token product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for timed/forever/capped sale mode.
...
- Validated state for timed/forever/capped sale mode.
...
- Standalone validation: open Gnocchi, exercise timed/forever/capped sale mode, assert visible status and no console-modal dependency.
