...
const INSTALLER_PLATFORMS = [
  {
    key: "macos",
    label: "macOS",
    env: "CH_EASE_INSTALLER_MACOS_URL",
    sha256Env: "CH_EASE_INSTALLER_MACOS_SHA256",
    fileName: "CH-EASE-Studio.dmg",
  },
  {
    key: "windows",
    label: "Windows",
    env: "CH_EASE_INSTALLER_WINDOWS_URL",
    sha256Env: "CH_EASE_INSTALLER_WINDOWS_SHA256",
    fileName: "CH-EASE-Studio.exe",
  },
  {
    key: "raspberry-pi",
    label: "Raspberry Pi",
    env: "CH_EASE_INSTALLER_RASPBERRY_PI_URL",
    sha256Env: "CH_EASE_INSTALLER_RASPBERRY_PI_SHA256",
    fileName: "ch-ease-studio-arm64.deb",
  },
] as const;
...
router.get("/api/ch-ease/installers", isAuthenticated, (_req, res) => {
  const version = String(process.env.CH_EASE_INSTALLER_VERSION || "").trim();
  return res.json({
    ok: true,
    product: "ch-ease",
