# Penne: Distribution token metadata setup

## Purpose

Distribution token metadata setup lets Pasta Protocol users move the Penne workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- The user can access `/tools/penne` and the relevant app gate is enabled.
...
- User-entered configuration for distribution token metadata setup.
- Wallet/network context when the feature signs or reads chain state.
- Package, media, metadata, contract, or relationship data appropriate to Penne.

## Outputs

- Validated state for distribution token metadata setup.
...
- Standalone validation: open Penne, exercise distribution token metadata setup, assert visible status and no console-modal dependency.
