...
Evidence snapshot: the latest recorded production verification is PR #28 final-launch-command commit `956420f`, with Deploy to Hetzner run `28566737184` passing after a cache-only disk-preflight recovery, main Quality Gates run `28566737178` passing, live health reporting production DB and mainnet chain health, `npm run pasta:repo-cleanup:audit` passing against `origin/main` after the promoted PR branch was pruned, and the named `npm run pasta:live-readiness:final` command proving all non-WTF.ME Pasta surfaces on live `956420f` before exiting `1` only on dedicated WTF.ME credentials and `PASTA_WTFME_LIVE_HOST`. PR #26 remains the cleanup-audit disappearing-ref hardening source; PR #28 is the current live evidence boundary. This is a production evidence snapshot, not a full Pasta launch claim.
...
| Unified installer catalog | Live `https://wtfos.app/api/pasta/installers/catalog` returned unauthenticated `401` after PR #14 deployed commit `04b242a`, on `cb74cc1`, on `9507502`, during the PR #16 verification snapshot at `2a1977e`, during the PR #17 verification snapshot at `26c60cd`, in the PR #22 production evidence snapshot at `984b3f5`, in the PR #23 production evidence snapshot at `92bb731`, in the PR #25 production evidence snapshot at `7657a3b`, in the PR #26 production evidence snapshot at `c1686b5`, under the PR #27 live proof at `4e50cf3`, and under the PR #28 live final-command proof at `956420f`; `npm run pasta:live-readiness` and `npm run pasta:live-readiness:final` verify this route before reporting only WTF.ME blockers. | PROVEN |
