...
- Evidence:
  ...
  - The current source authority is `origin/main` / local `main` at `c3b445ba5ee43831bd99f4080560adc5a713d9f5` (`Track generated MapLab demo totals in Gamma spec instead of curated counts`).
...
- Evidence:
  ...
  - 2026-07-06 current-main recheck: local `main` and `origin/main` both point at `c3b445ba`, `git worktree list --porcelain` reports only the primary WTF checkout, and `npm run pasta:repo-cleanup:audit` passes with no branch/worktree blockers. The current dirty tree is valid ongoing split work, while `codex/pasta-live-readiness` and `wip/full-working-tree-20260617` are missing historical refs that must not be recreated or replayed wholesale.
...
- Correction:
  ...
  - Treat the current dirty `main` checkout as valid ongoing work to split into release lanes, not as a stale checkout to reset, mine, or archive.
