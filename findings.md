...
The current pass treats the user's "Wikipedia" critique as the highest-priority onboarding bug. `/beta` now boots into an active command window first, not a readable world-status essay: the oversized "Live now" panel is reduced to secondary status, the current quest owns the first action, and people/object/return proof lives as three command buttons inside the active window. Mobile now sees the command surface before the world-status rail, so the first screen starts with what to do rather than what to read.

The newest polish pass moves beta another step away from "Wikipedia with a skin." The first screen now uses a compact world-status panel beside the command surface, a smoother shell status bar, and a full-width live desktop band instead of a floating explanatory block. The launcher labels are short product controls, the people panel says "People are here" instead of describing the proof system, and the Admin Lab remains the intentional place for maps, runbooks, and evidence.

The latest user critique was correct: a beta shell can have accurate route maps and still fail if it feels like Wikipedia. The current pass changes the default surface from "read the ecosystem" to "enter the OS": an active quest window, embedded live people/object/return doors, a compact session HUD, a visible NOW state on the current quest stage, a closed Admin Lab for evidence, and a visible next workspace on desktop. On mobile, the active command window now appears before world-status context, and the smallest layout hides the duplicate live rail. This keeps the reports available without letting them define the product.
