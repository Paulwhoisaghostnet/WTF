...
const PRODUCT_NAME = "CH-EASE Studio";
...
function safeStaticPath(urlPath) {
  ...
  const fullPath = path.join(chEaseRoot(), rel);
  const root = chEaseRoot();
...
async function handleRequest(req, res) {
  ...
  if (req.method === "GET" && parsed.pathname === "/api/auth/user") {
    return sendJson(res, 401, {
      error: "CH-EASE Desktop does not include wtfOS hosted resources.",
      native: true,
      product: "ch-ease-studio",
    });
  }
  if (req.method === "GET" && parsed.pathname === "/api/auth/csrf-token") {
    return sendJson(res, 200, { csrfToken: "ch-ease-desktop" });
  }
  if (req.method === "GET" && parsed.pathname === "/api/ch-ease/installers") {
  ...
  if (
    ...
    return sendJson(res, 403, {
      error: "wtfOS hosted pinning, publishing, and preview processing are not available in CH-EASE Desktop. Use Pinata or your own IPFS node.",
      native: true,
      product: "ch-ease-studio",
    });
  }
...
function createWindow() {
    ...
    if (shouldAllowPopup(url)) {
      return {
        ...
        overrideBrowserWindowOptions: {
          ...
          title: url.startsWith(baseUrl) ? "CH-EASE publisher" : "CH-EASE wallet",
          webPreferences: {
            nodeIntegration: false,
            contextIsolation: true,
            sandbox: true,
  ...
  mainWindow.loadURL(`${baseUrl}/creation-tools/ch-ease/index.html`);
}
...
if (!hasSingleInstanceLock) {
  ...
  app.whenReady().then(async () => {
    try {
      ...
      dialog.showErrorBox("CH-EASE Studio failed to start", err.message || String(err));
      app.quit();
    }
