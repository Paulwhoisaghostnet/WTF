...
| Social auth | Public/session callback | Start and complete Google, GitHub, X/Twitter, Discord, and AT Protocol OAuth; view provider config; run X OAuth diagnostics/self-test; publish AT Protocol OAuth client metadata. | `auth.oauth.started`, `auth.oauth.completed`, `auth.oauth.failed`, `auth.oauth.diagnostics.viewed`, `atproto.account.linked`, `atproto.account.unlinked` |
...
| Domain | Access | Possible interactions | Primary handles |
| --- | --- | --- | --- |
| Tezos wallets | Session | List linked wallets; passively rehydrate cached local wallet display state without requesting ownership proof; request challenge only from explicit connect/link or participation flows; link connected/manual address; unlink; set primary; sync profile tokens; sync one wallet; fetch wallet WTF balance/tokens/profile dossier. | `wallet.challenge.created`, `wallet.linked`, `wallet.unlinked`, `wallet.primary_set`, `wallet.sync_requested`, `wallet.balance.viewed` |
...
| Calendar/events | Public/session/staff/bot | Browse merged TTC, WTF, and browser-local personal events/feed with TTC treated as the higher-ranked source of truth; submit browser-local personal entries; submit WTF tickets; open TTC event submission in an in-app iframe; view own tickets; staff review/publish/cancel/manual-create/sync WTF events; bot mirrors Discord events. | `calendar.viewed`, `calendar.ttc_feed_pulled`, `calendar.personal_entry_created`, `calendar.ttc_submit_opened`, `calendar.ticket_submitted`, `calendar.ticket_reviewed`, `calendar.event_published`, `calendar.event_cancelled`, `discord.event_mirrored` |
...
| Skywire AT Protocol bridge | Session/system | Open the native Skywire app; register a new AT Protocol identity on an allowed PDS; request PDS phone verification and submit the returned code when required; hand off to the official PDS signup flow when the selected PDS requires phone verification but does not expose phone-code requests to Skywire; connect an existing Bluesky/AT Protocol account through OAuth; store DID/handle/profile/PDS and encrypted OAuth/credential material server-side only; update the AT profile; search actors, inspect author feeds, and follow actors; distinguish AT-compliant handles from Tezos aliases; claim or verify WTF-hosted/DNS/HTTPS handles; serve public `/.well-known/atproto-did`; read WTF/Tezos/mentions feeds; compose, like, repost, reply, receive notifications, and claim existing posts for challenge automation; publish WTF-native Skywire Signal records into the user's AT repo for portable quest/drop/proof/broadcast state beyond Bluesky's app surface; use Bluesky compose intent fallback without auto-posting. | `atproto.phone_verification.requested`, `atproto.external_signup.opened`, `atproto.account.registered`, `atproto.account.linked`, `atproto.account.unlinked`, `atproto.profile.updated`, `atproto.actor.searched`, `atproto.actor.followed`, `atproto.handle.claimed`, `atproto.handle.verified`, `atproto.signal.published`, `atproto.post.created`, `atproto.post.claimed`, `atproto.post.replied`, `atproto.post.liked`, `atproto.post.reposted`, `atproto.notification.received` |
