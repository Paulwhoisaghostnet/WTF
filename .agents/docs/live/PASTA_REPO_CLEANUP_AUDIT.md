...
2. Publish the generated Pasta pages to a real claimed WTF.ME host and make `PASTA_WTFME_LIVE_HOST=<published-host> npm run pasta:wtfme:live-check` pass through page markers, `.well-known/wtfos-pins`, DID resolution, and the PDS `pinManifest` record.
