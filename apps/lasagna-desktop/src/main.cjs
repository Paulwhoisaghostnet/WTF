...
const PRODUCT_NAME = "Lasagna Studio";
...
function lasagnaRoot() {
  return path.join(appRoot(), "lasagna");
}
...
function safeStaticPath(urlPath) {
  ...
  const fullPath = path.join(lasagnaRoot(), rel);
  const root = lasagnaRoot();
...
async function handleRequest(req, res) {
  ...
  if (req.method === "GET" && parsed.pathname === "/api/auth/user") {
    return sendJson(res, 401, {
      error: "Lasagna Desktop does not include wtfOS hosted resources.",
      native: true,
      product: "lasagna-studio",
    });
  }
  if (req.method === "GET" && parsed.pathname === "/api/auth/csrf-token") {
    return sendJson(res, 200, { csrfToken: "lasagna-desktop" });
  }
  if (req.method === "GET" && parsed.pathname === "/api/lasagna/installers") {
  ...
  if (
    ...
    return sendJson(res, 403, {
      error: "wtfOS hosted pinning, publishing, and preview processing are not available in Lasagna Desktop. Use Pinata or your own IPFS node.",
      native: true,
      product: "lasagna-studio",
    });
  }
...
if (!hasSingleInstanceLock) {
  ...
  app.whenReady().then(async () => {
    try {
      ...
      dialog.showErrorBox("Lasagna Studio failed to start", err.message || String(err));
      app.quit();
    }
