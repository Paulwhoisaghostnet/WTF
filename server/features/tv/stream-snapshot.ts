  ...
  if (shuffledRows.length === 0) {
    if (shuffledBumpers.length > 0) {
      const bumperQueue: TvStreamQueueItem[] = shuffledBumpers.map((b, index) => ({
        ...
        mimeType: b.mimeType,
        thumbnailUri: null,
        sourceUri: `/api/tv/bumpers/${b.id}/media?channelId=${channelId}`,
        cacheUrl: `/api/tv/bumpers/${b.id}/media?channelId=${channelId}`,
  ...
  shuffledRows.forEach((row, index) => {
    ...
    if (bumperEnabled && (videosSinceBumper >= cadence || atLastItem)) {
      ...
      queue.push({
        ...
        thumbnailUri: null,
        sourceUri: `/api/tv/bumpers/${bumper.id}/media?channelId=${channelId}`,
        cacheUrl: `/api/tv/bumpers/${bumper.id}/media?channelId=${channelId}`,
