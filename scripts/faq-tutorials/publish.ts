...
import catalog from "../../shared/faq-tutorials.json";
import {
...
async function main(): Promise<void> {
  ...
  const sourceDir = path.resolve(process.argv[2] || "output/faq-tutorials/final");
  const verificationDir = await fs.mkdtemp(path.join(tmpdir(), "wtfos-faq-tutorials-"));
  const published: Array<{ slug: string; key: string; bytes: number; sha256: string }> = [];

  try {
    for (const tutorial of catalog) {
      const assets = [
        { extension: "mp4", key: tutorial.videoObjectKey, contentType: "video/mp4" },
        { extension: "vtt", key: tutorial.captionsObjectKey, contentType: "text/vtt; charset=utf-8" },
        { extension: "jpg", key: tutorial.posterObjectKey, contentType: "image/jpeg" },
      ];
      for (const asset of assets) {
        const filePath = path.join(sourceDir, `${tutorial.slug}.${asset.extension}`);
        ...
        const verificationPath = path.join(verificationDir, `${tutorial.slug}.${asset.extension}`);
        ...
        published.push({ slug: tutorial.slug, key: asset.key, bytes: stat.size, sha256: digest });
        console.log(`[publish] verified s3://${storage.bucket}/${asset.key}`);
      }
    }
    await fs.writeFile(
