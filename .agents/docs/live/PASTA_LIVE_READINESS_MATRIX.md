...
Release lane: `origin/main` at live commit `9507502`
...
| Unified installer catalog | Live `https://wtfos.app/api/pasta/installers/catalog` returned unauthenticated `401` after PR #14 deployed commit `04b242a`, on `cb74cc1`, and again on current live commit `9507502`; `PASTA_LIVE_READINESS_ALLOW_BLOCKERS=1 npm run pasta:live-readiness` now verifies this route before reporting only WTF.ME blockers. | PROVEN |
