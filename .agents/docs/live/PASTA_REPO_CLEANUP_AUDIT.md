...
- Authoritative production source is current `origin/main` at `0558be0e217950f0f45fd2f7f9da886aefaf84e0` (`Prepare Pasta standalone installer release lane`).
- Live `https://wtfos.app/api/health` returned `status: "ok"`, `version.commitRef: "0558be0"`, `nodeEnv: "production"`, healthy database, and current mainnet chain defaults after the standalone installer publication pass.
- Clean promotion branch `codex/spaghetti-installer-live` has been merged into `main` for the scoped Pasta installer/readiness lane. Its remaining local delta is a post-publication audit/docs fix: update the standalone installer release audit to use the GitHub Actions API workflow list, and refresh live evidence docs after Gnocchi/Ravioli/Rotini/Penne/Lasagna release publication.
- Historical Pasta release milestones remain ancestors of `0558be0`: Spaghetti installer route commit `09fff2fb`, Spaghetti live verification `e080b89`, stale-worktree cleanup `21e1aca`, and Shadownet preflight `24cf9e2`.
- Main Quality Gates and Deploy to Hetzner passed for those historical milestones, and the later standalone installer merge/deploy supersedes them as the current live authority.
...
| `.config/superpowers/worktrees/WTF/codex-spaghetti-installer-live` | `codex/spaghetti-installer-live` | 7 | Current Pasta audit/docs continuation checkout | Merged release lane with a narrow post-publication audit/docs delta. Promote only the standalone audit API fix plus evidence-doc refresh from here; full Pasta remains blocked by live WTF.ME host and pin-discovery proof, not by individual installer publication. |
...
- Pasta suite installer/download is now production-complete for the bundled native suite lane: `apps/pasta-suite-desktop`, the `pasta-suite-desktop-installers.yml` workflow, `/api/pasta/installers`, inventory coverage, package policy checks, GitHub release assets, production env, and the authenticated live verifier all passed for `pasta-suite-desktop-v1.0.0`. The source manifest contract now requires the suite manifest to enumerate Macaroni, Spaghetti, Gnocchi, Ravioli, Rotini, Penne, and Lasagna in `bundledApps`; keep `npm run pasta-suite:installers:live-check` in release gates so production continues serving that strengthened contract.
...
- Release workflow proof: main commit `0558be0` registered the five GitHub Actions installer workflows; workflow runs Gnocchi `28519193761`, Ravioli `28519193772`, Rotini `28519193756`, Penne `28519193792`, and Lasagna `28519193803` passed and published `*-desktop-v1.0.0` release tags.
...
- Release-gate proof: `PASTA_LIVE_READINESS_ALLOW_BLOCKERS=1 npm run pasta:live-readiness` passed its executable audit mode on 2026-07-01 by proving live health, live static bundle/runtime markers, public Macaroni/Pasta Suite/Spaghetti/Gnocchi/Ravioli/Rotini/Penne/Lasagna installer release assets, and the recorded Colander action proof while reporting the expected blockers: no local Pasta WTF.ME publish credentials and no proven live Pasta WTF.ME host. Source policy now requires the gate to run `pasta:wtfme:live-publish` with `PASTA_WTFME_LIVE_PUBLISH=0` and host binding when credentials are supplied.
