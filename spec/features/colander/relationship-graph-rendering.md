# Colander: Relationship graph rendering

## Purpose

Relationship graph rendering lets Pasta Protocol users move the Colander workflow toward a valid management outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for relationship graph rendering.
...
- Validated state for relationship graph rendering.
...
- Standalone validation: open Colander, exercise relationship graph rendering, assert visible status and no console-modal dependency.
