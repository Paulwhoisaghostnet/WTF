...
function debit(walletId: string, totalMutez: number): boolean {
  const gameState = ensureState();
...
export function joinRugPullRound(rawUser: ConsoleAuthUser, nowMs = now()) {
  ...
  const user = walletForUser(rawUser);
  const round = gameState.round;
  if (round.phase !== "active") return { ok: false, error: "Round is not joinable.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
  if (round.players[user.walletId]) return { ok: false, error: "You are already in this round.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
  const split = splitRugPullPayment("join");
  if (!debit(user.walletId, split.totalMutez)) return { ok: false, error: "Insufficient mocked XTZ balance.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
...
export function delayRugPullButton(rawUser: ConsoleAuthUser, nowMs = now()) {
  ...
  if (round.phase !== "active" || !player || player.status !== "active" || nextCost == null) {
    return { ok: false, error: "Delay is not available.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
  }
  if (!canWalletCauseButtonLock({ walletAddress: user.walletId, lastLockWalletAddress: round.lastLockWalletId })) {
    return { ok: false, error: "Same wallet cannot cause two button locks in a row.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
  }
  if (!debit(user.walletId, nextCost)) return { ok: false, error: "Insufficient mocked XTZ balance.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
  player.delayCount += 1;
...
export function pressRugPullButton(rawUser: ConsoleAuthUser, nowMs = now()) {
  ...
  if (!player || player.status !== "active") return { ok: false, error: "Only active unpressed players can press.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
  if (round.phase === "active" && round.buttonLockUntilMs > nowMs) return { ok: false, error: "Button is locked.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
  if (round.phase !== "active" && round.phase !== "panic") return { ok: false, error: "Round is not pressable.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
  if (!debit(user.walletId, split.totalMutez)) return { ok: false, error: "Insufficient mocked XTZ balance.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
...
export function joinRugPullWitness(rawUser: ConsoleAuthUser, nowMs = now()) {
  ...
  const user = walletForUser(rawUser);
  const round = gameState.round;
  if (round.witnesses[user.walletId]) return { ok: false, error: "You are already a witness.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
...
export function voteRugPullWitness(rawUser: ConsoleAuthUser, vote: RugPullVote, nowMs = now()) {
  ...
  if (round.phase !== "panic" || !witness || witness.vote) {
    return { ok: false, error: "Witness vote is not available.", snapshot: getRugPullSnapshot(rawUser, nowMs) };
  }
  witness.vote = vote;
