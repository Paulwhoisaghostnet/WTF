# Penne: Allocation validation and dedupe

## Purpose

Allocation validation and dedupe lets Pasta Protocol users move the Penne workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for allocation validation and dedupe.
...
- Validated state for allocation validation and dedupe.
...
- Standalone validation: open Penne, exercise allocation validation and dedupe, assert visible status and no console-modal dependency.
