...
const kilnClient = readFileSync("server/lib/kiln-client.ts", "utf8");
...
test("collection factory uses Docker-safe Kiln defaults and token aliases", () => {
  assert.match(route, /from "\.\.\/lib\/kiln-client"/);
  assert.match(route, /kilnFetch/);
  assert.match(kilnClient, /process\.env\.NODE_ENV === "production"[\s\S]*host\.docker\.internal:3001/);
  assert.match(kilnClient, /process\.env\.KILN_API_TOKEN\?\.trim\(\)/);
  assert.match(kilnClient, /process\.env\.WTF_KILN_API_TOKEN\?\.trim\(\)/);
  assert.match(kilnClient, /process\.env\.API_AUTH_TOKEN\?\.trim\(\)/);
  assert.match(kilnClient, /process\.env\.KILN_TIMEOUT_MS/);
  assert.match(kilnClient, /process\.env\.WTF_KILN_TIMEOUT_MS/);
  assert.match(kilnClient, /AbortController/);
  assert.match(kilnClient, /controller\.abort\(\)/);
  assert.match(kilnClient, /assertKilnConfigured/);
  assert.match(
    dockerCompose,
...
test("deployment preflights protected Kiln mutation auth before app restart", () => {
  ...
  assert.match(deploy, /node scripts\/check-kiln-auth\.mjs[\s\S]*check-kiln-production-posture\.mjs[\s\S]*docker compose build/);
