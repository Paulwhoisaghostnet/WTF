...
import {
    fetchWtfProfileTokenCollection,
    isWtfProfileGallery,
    WtfProfileTokenFetchError,
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (isWtfProfileGallery(address) || searchParams.get("source") === "wtf") {
            const result = await fetchWtfProfileTokenCollection({
                page,
                pageSize,
                forceRefresh,
        ...
        if (error instanceof WtfProfileTokenFetchError) {
            return NextResponse.json(
                {
                    success: false,
                    error: error.message,
