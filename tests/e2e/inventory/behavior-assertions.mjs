export const CORE_BEHAVIOR_ASSERTIONS = [
  ...
  },
  {
    ...
    verificationCommand: "npx tsx --test client/src/lib/wallet-context-policy.test.ts client/src/pages/profile-wallet-link-policy.test.ts",
    userVisibleAssertion:
  ...
  },
  {
    ...
    userVisibleAssertion:
      "A trusted-creator puppet can open Macaroni, enter the Studio through the embedded creation-tool iframe, see Shadownet as the default rehearsal network, see the wtfOS IPFS provider and wtfOS Publish button, see Fileship as the default IPFS gateway, choose Macaroni V1 or V2 contract templates, configure per-token edition quantities, optional minter royalty pool policy, and delayed-reveal unrevealed placeholder image pools, see the 1 GB per-artifact hard max, 250 MB average artifact limit, and 1 MB square JPG/PNG collection logo/cover limit, connect a Shadownet puppet wallet without RPC errors, show the Beacon picker with Kukai and Temple options on explicit connect, route selected Kukai pairing to the Shadownet Kukai app, receive sandbox-safe inline validation/deploy feedback instead of blocked browser modals while Studio keeps deploy/sync signing on the same chain-guarded path for Shadownet and mainnet, clear the current Studio draft into a new drop without disconnecting the wallet, restore a project from backup JSON, visibly save sale-stage edits, block wtfOS publish until the drop has a deployed or resumed KT1 contract, and distinguish live wtfOS publish URLs from PDS-delivered-but-pending `.me` serving; a regular signed-in puppet can still use the Studio deploy/export flow but does not see the wtfOS IPFS provider or wtfOS Publish button, and sees self-host export as the normal publishing path; generated mint pages expose clean disconnect, prevent duplicate request-permission flows from rapid connect clicks or stale passive restore, include basic accessibility landmarks/status/progress/quantity semantics, surface minter royalty metadata sync/lock status when V2 minter royalties are enabled, label a one-stage live sale as `Mint is Live` and multi-stage live sales as `Currently on Sale Stage X of N`, title the wallet gallery as drop tokens, show minted-this-stage versus max-per-wallet and currently-owned counts separately, order wallet cards with tokens minted by the connected wallet first and tokens owned from other minters second, add prefilled X and Bluesky compose share buttons on wallet token cards that mention the creator's matching platform handle when available, use editable Studio share copy/handle overrides, include the token media URL, and add blind-mint-level X and Bluesky compose buttons that share the current sale stage, mint cost, per-wallet/access terms, supply, drop URL, and collection cover image URL, clamp requested mint quantity to live collection remaining supply plus the connected wallet's remaining per-wallet/allowlist allowance, align the Beacon active-account RPC before signing, derive explicit wallet fees from the padded gas limit actually sent to Taquito/Beacon, distinguish wallet-returned operation hashes from node-visible mempool/indexer acceptance, classify confirmation polling timeouts by operation hash before showing safe retry guidance, show wallet balance/cost status, max-per-wallet status, owned-token recovery hooks, a public Recent mints section with minter identity links, refreshed contract-backed token metadata, video-capable token previews, and bounded theme styling instead of arbitrary stored CSS; published snapshot compatibility also forces custom recent-mint grids to read actual TzKT mint transfer token IDs instead of inferring recent tokens from mint count.",
    durableSideEffectAssertion:
      "The focused runner seeds dummy users and Shadownet puppet wallet metadata, verifies the live Shadownet RPC chain id `NetXsqzbfFenSTS` in the Macaroni iframe, proves the IPFS provider selector and wtfOS Publish button follow `trusted_market_creator` access, proves a mismatched RPC is blocked before wallet signing, confirms explicit connect does not open a wallet handoff before the Beacon picker is visible, checks from `/tools/macaroni` that Beacon's real Kukai option can escape the sandbox and load `https://shadownet.kukai.app` instead of a blank, mainnet, or Temple-only tab, asserts rapid generated-page connect clicks coalesce to one Beacon permission request, and keeps source-policy coverage for the Macaroni V1/V2 contract selector, compiled V2 SmartPy artifact, edition quantities, minter royalty metadata updater/lock policy, placeholder pool exports, 1 GB per-file and 250 MB average Macaroni artifact policy, direct-origin hosted upload tickets for Cloudflare-bypassing media pinning, OBJKT-sized per-token GIF/video preview generation, sandbox-safe Studio notices without native `alert`/`confirm`, shared mainnet/Shadownet deploy-path parity, optional treasury/royalty address normalization, new-drop draft reset, backup JSON restore, sale-stage save confirmation, generated mint-page live stage copy, minted-versus-owned wallet gallery ordering and count copy, social share creator-handle, token-media, and blind-mint sale compose text, Beacon active-account RPC alignment, wallet fee floor alignment with the padded gas limit, wallet-returned-hash versus node-visible operation status copy, operation-hash-aware confirmation timeout classification, contract-required wtfOS publishing with live-versus-pending `.me` serving status, published user-site WalletConnect relay websocket and verify-frame CSP allowances, stored Macaroni snapshot current-runtime compatibility, custom recent-mint grid compatibility using TzKT transfer token IDs, non-image fallback cover metadata, installer manifest visibility, the generated mint page's validated wallet restore, disconnect, accessible controls/status regions, balance preflight, max-per-wallet and allowlist remaining allowance clamping, TzKT-owned-token lookup, TzKT minted-token transfer lookup, TzKT recent-mint transfer lookup with contract metadata refresh/retry, Objkt/TzKT minter identity fallback, Fileship gateway default, and CSS theme allowlist paths.",
  },
  {
