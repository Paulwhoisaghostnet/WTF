...
Evidence snapshot: the latest recorded production verification is PR #30 deploy-cache-recovery commit `6f71f14`, with Deploy to Hetzner run `28570029612` passing, main Quality Gates run `28570029603` passing, live health reporting production DB and mainnet chain health, `npm run pasta:repo-cleanup:audit` passing against `origin/main` after the promoted PR branch was pruned, and the named `npm run pasta:live-readiness:final` command proving all non-WTF.ME Pasta surfaces on live `6f71f14` before exiting `1` only on dedicated WTF.ME credentials and `PASTA_WTFME_LIVE_HOST`. PR #26 remains the cleanup-audit disappearing-ref hardening source; PR #28 introduced the strict final-launch command; PR #30 is the current live evidence boundary and verifies the deploy disk preflight now has cache-only recovery while still avoiding broad Docker prune commands. This is a production evidence snapshot, not a full Pasta launch claim.
