...
| Agents/automation | `/mcp`, `/api/mcp/tokens`, Discord bot routes, attendance bot routes, board webhooks, client logs, scheduled Arcade source import, wallet/indexing workers. |
...
| Local auth | Public/session | Register; log in; log out; fetch current user; change or set password; handle failed or expired sessions. | `auth.register.succeeded`, `auth.register.failed`, `auth.login.succeeded`, `auth.login.failed`, `auth.logout`, `auth.password.changed` |
...
| Profile picture | Session/owner | Browse owned-token PFP candidates; search/page candidates; edit token-backed PFP on canvas; save or remove PFP. | `profile.pfp.candidate_viewed`, `profile.pfp.edited`, `profile.pfp.saved`, `profile.pfp.removed` |
...
| Domain | Access | Possible interactions | Primary handles |
| --- | --- | --- | --- |
| Window shell | Public/session by route | Open windows from routes/icons/Start Menu; focus existing windows; use full-screen auth overlays; close/minimize/focus windows. | `desktop.window.opened`, `desktop.window.focused`, `desktop.window.closed`, `desktop.start_menu.opened` |
| Desktop icons | Session/public gates | Open enabled app icons; move/reposition icons; reset layout; respect admin app gates. | `desktop.icon.opened`, `desktop.icon.moved`, `desktop.icon_layout.reset` |
...
| MCP pairing | Session | View MCP endpoint; create named token; copy raw token once; list active prefixes/scopes/last-use; revoke token. | `mcp.token.created`, `mcp.token.revoked`, `mcp.token.listed` |
...
| Challenges | Public/session/staff | List/filter/view; inspect reward/subdomain/mint data; submit text/link/evidence; view own submission; staff create/update/grade pass/fail/bonus, set reward flags, mark reward paid. | `challenge.viewed`, `challenge.submitted`, `challenge.graded`, `challenge.reward_flag.created`, `challenge.reward_claimed`, `challenge.reward_marked_paid` |
...
| Side quests | Public/session/staff | List/view active/completed quests; submit completion evidence; run auto-verify; view own completions; staff create/update quests and approve/reject completions. | `side_quest.viewed`, `side_quest.completion_submitted`, `side_quest.auto_verified`, `side_quest.approved`, `side_quest.rejected` |
| Side quest rewards | System/staff | Award quest XP; insert reward ledger row; grant wtf.tez subdomain; enforce max completions/deadlines/persistence. | `xp.awarded`, `reward.ledger.created`, `wtf_domain.granted`, `side_quest.reward_distributed` |
...
| Challenge automation | Staff/system | Admin builds event/predicate rules from registered trigger handles, combines ALL/ANY conditions, attaches reward actions, activates/pauses/archives definitions, inspects per-user progress, recent events, action logs, and audit logs. Live ingestion currently covers messageboard posts/channel posts, XP awards, wallet links/relinks, desktop pet interactions, and generic `app.interaction.tracked`; Tezos ownership predicates verify against linked wallet holdings/TzKT. | `messageboard.post.created`, `messageboard.channel.post.created`, `xp.awarded`, `wtf.awarded`, `user.wallet.connected`, `desktop.pet.interacted`, `app.interaction.tracked`, `nft.ownership.verified`, `token.contract.owned`, `token.id.owned`, `gameshow.challenge.completed` |
...
| Leaderboards | Public | View WTF holder leaderboard; view XP leaderboard; view recent WTF transfers with Tezos enrichment. | `leaderboard.viewed`, `leaderboard.xp.viewed`, `leaderboard.transfers.viewed` |
...
| Reward ledger/operator | Staff | View/filter/select rewards; mark paid; batch pay with op hash; preview/run/reconcile operator wallet disbursements. | `reward.ledger.viewed`, `reward.marked_paid`, `operator.disbursement.previewed`, `operator.disbursement.run`, `operator.disbursement.reconciled` |

## Concern: Community, Social, Messaging, and Discord

| Domain | Access | Possible interactions | Primary handles |
| --- | --- | --- | --- |
| Message board | Public/session/staff/webhook | List categories/channels; read channel messages/pins; post where allowed; edit/delete own; staff moderate, pin/unpin, manage categories/channels/permissions/webhooks. | `board.channel.viewed`, `board.message.created`, `board.message.edited`, `board.message.deleted`, `board.message.pinned`, `board.webhook_received`, `xp.awarded` |
...
| Notifications | Session | View tab; toggle settings; filter all/unread; open linked item; mark one/all read. | `notification.viewed`, `notification.opened`, `notification.read`, `notification.preference.updated` |
| W timeline | Session | View cached/live timeline; refresh; compose post; upload media; reply; like; repost; quote; open post detail; view diagnostics/source/staleness; use link preview. | `w.timeline.viewed`, `w.post.created`, `w.media.uploaded`, `w.reply.created`, `w.like.created`, `w.repost.created`, `w.quote.created`, `w.link_preview.requested` |
| W chats/DMs | Session/staff | View official groupchats; send groupchat messages; list user DMs; read/send DMs; start direct DM; staff manage official groupchat IDs and stream rules. | `w.groupchat.viewed`, `w.groupchat.message_sent`, `w.dm.viewed`, `w.dm.sent`, `w.admin.stream_rule.updated` |
| W social/settings | Session/staff | View follows summary; list/follow accounts; inspect X Spaces; embed X Space URL; view capabilities; inspect stream status and DM diagnostics. | `w.follow.created`, `w.spaces.viewed`, `w.space.embedded`, `w.capabilities.viewed`, `w.diagnostics.viewed` |
...
| Game Studio templates/assets | Public/session | Fetch templates, targets, stock assets, snippets, raw stock asset files, and scaffold source files. | `game_studio.template.viewed`, `game_studio.asset.viewed`, `game_studio.snippet.viewed`, `game_studio.scaffold.created` |
...
| Studio workspace | Session/permission | Create/list/open projects; connect/disconnect Google Drive; refresh usage; upload files/folders; preview/download/rename/delete files; create/resolve/reopen/delete annotations; comment; chat; pin/unpin; invite members; presence. | `studio.project.created`, `studio.project.opened`, `studio.file.uploaded`, `studio.file.renamed`, `studio.file.deleted`, `studio.annotation.created`, `studio.annotation.resolved`, `studio.chat.sent`, `studio.member.invited` |
...
| In-app inventory use | Session/owner/API | Consume food/medicine through `/api/in-app-market/use`; consume Arcade play ticket on Arcade session; enforce pet-ball cap and stock limits. | `inventory.item.consumed`, `inventory.item.granted`, `inventory.stock_reserved`, `inventory.cap_blocked` |
...
| Casino table registry and wagers | Session/system | List installed table stubs; keep wagering disabled until each game has a domain-owned wager-session contract, house-take policy, settlement verifier, replay guard, and durable audit log. | `casino.games.viewed`, `casino.wager_session.rejected`, `casino.entry.rejected` |
...
| Rug Pull: The Game | Session/wallet/system | Open `/casino/rug-pull`; view the active mocked round, current pot, next-round seed, pressure multiplier, button lock, Panic Mode countdown, witness vote state, player share table, and settlement feed; submit mocked join, delay, press, witness, and witness-vote actions through deterministic service interfaces until the dedicated Tezos contract and verifier replace the mock adapter. | `rug_pull.rules.viewed`, `rug_pull.round.join_intent_created`, `rug_pull.button.delay_intent_created`, `rug_pull.button.press_intent_created`, `rug_pull.witness.join_intent_created`, `rug_pull.witness.vote_cast`, `rug_pull.round.settled`, `rug_pull.wager.rejected` |
| Guinea Pig Raceway | Session/wallet/system | Open `/casino/guinea-pig-raceway`; view the mocked live race card, 3D GLB racer scene, racer stats, five-track catalog data, conditions, global variables, lockout schedule, live race phase, paid effect caps, result replay metadata, tote board, and replay camera angles; submit mocked bet/effect actions only through deterministic service interfaces until the raceway contract, randomness beacon, replay archive, and settlement verifier replace the mock adapter. | `guinea_pig_raceway.rules.viewed`, `guinea_pig_raceway.race_card.viewed`, `guinea_pig_raceway.bet_intent_created`, `guinea_pig_raceway.bet_locked`, `guinea_pig_raceway.bet_rejected`, `guinea_pig_raceway.intro.started`, `guinea_pig_raceway.race.started`, `guinea_pig_raceway.effect_intent_created`, `guinea_pig_raceway.effect_rejected`, `guinea_pig_raceway.race.settled`, `guinea_pig_raceway.replay.viewed`, `guinea_pig_raceway.wager.rejected` |
...
| WTF Arcade play | Session | View play status/ticket count/bypass; create WTF payment intent; create signed play session after ticket/bypass; submit one-use signed score; receive XP for valid score events. | `arcade.play_status.viewed`, `arcade.play_intent.created`, `arcade.play_ticket.consumed`, `arcade.session.created`, `arcade.score.submitted`, `arcade.score.accepted`, `arcade.score.rejected`, `xp.awarded` |
...
| WTF Arcade source import | Staff/system | Run compatible-source ingest/check; scheduled worker records imported/skipped/updated rows and audit health even when no games changed. | `arcade.source_import.started`, `arcade.source_import.completed`, `arcade.source_import.failed`, `arcade.audit.created` |
...
| Game Studio SDK | Public/session/agent | Generate scaffold from templates; use SDK hooks `ready`, `startSession`, `updateScore`, `gameOver`, `pause`; build SDK-compatible ZIP; submit project to Arcade. | `game_sdk.ready`, `game_sdk.session_started`, `game_sdk.score_updated`, `game_sdk.game_over`, `game_sdk.paused` |
...
| Roles/permissions/app gates | Admin | View/toggle role permissions; reset permission set; view/update desktop app gates and gate-aligned MCP feature access. | `admin.permissions.updated`, `admin.permissions.reset`, `admin.app_gate.updated` |
...
| MCP transport | Agent | Connect to `/mcp`; call tools with bearer token; receive rate-limit and feature-gate data; fail closed on revoked token/gate. | `mcp.connected`, `mcp.tool.called`, `mcp.tool.succeeded`, `mcp.tool.failed`, `mcp.rate_limit.hit` |
...
| Social spam | Board/DM/W post bursts, slow-mode hits, webhook abuse, media attachment spam, repeated deleted/moderated posts. | `board.message.created`, `dm.message.sent`, `w.post.created`, `board.webhook_received`, `system.rate_limit.hit` |
