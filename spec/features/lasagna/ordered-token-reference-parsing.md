# Lasagna: Ordered token reference parsing

## Purpose

Ordered token reference parsing lets Pasta Protocol users move the Lasagna workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- The user can access `/tools/lasagna` and the relevant app gate is enabled.
...
- User-entered configuration for ordered token reference parsing.
- Wallet/network context when the feature signs or reads chain state.
- Package, media, metadata, contract, or relationship data appropriate to Lasagna.

## Outputs

- Validated state for ordered token reference parsing.
...
- Standalone validation: open Lasagna, exercise ordered token reference parsing, assert visible status and no console-modal dependency.
