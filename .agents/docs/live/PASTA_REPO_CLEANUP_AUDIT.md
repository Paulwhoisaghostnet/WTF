...
- Tooling proof: `scripts/pasta-protocol/wtfme-live-publish.ts` defaults to dry-run and requires `PASTA_WTFME_LIVE_PUBLISH=1` plus scoped auth and CSRF before writing; it now checks the production TLS gate before calling the Pasta pin recovery publish route so a denied host cannot create fresh pin rows. `scripts/pasta-protocol/wtfme-live-inventory.ts` is read-only; `scripts/pasta-protocol/wtfme-live-check.ts` verifies the real host, page markers, wallet-safe headers, and `.well-known/wtfos-pins`.
