...
| Collector market | A collector needs object and reward context before market urgency. | `/gallery` -> `/side-quests` -> `/hoard` -> `/wtfiam` -> `/marketplace` -> `/rat-race` | Public object proof comes before spend prompts, market state keeps uncertainty visible, and reward sinks are capped. |
