...
| Skywire AT Protocol bridge | Session/system | Open the native Skywire app; connect an existing Bluesky/AT Protocol account through OAuth; store DID/handle/profile/PDS and encrypted OAuth/DPoP material server-side only; distinguish AT-compliant handles from Tezos aliases; claim or verify WTF-hosted/DNS/HTTPS handles; serve public `/.well-known/atproto-did`; read WTF/Tezos/mentions feeds; compose, like, repost, reply, receive notifications, and claim existing posts for challenge automation; use Bluesky compose intent fallback without auto-posting. | `atproto.account.linked`, `atproto.account.unlinked`, `atproto.handle.claimed`, `atproto.handle.verified`, `atproto.post.created`, `atproto.post.claimed`, `atproto.post.replied`, `atproto.post.liked`, `atproto.post.reposted`, `atproto.notification.received` |
