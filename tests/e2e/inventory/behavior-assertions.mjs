export const CORE_BEHAVIOR_ASSERTIONS = [
  ...
  },
  {
    ...
    userVisibleAssertion:
      "A trusted-creator puppet can open Macaroni, enter the Studio through the embedded creation-tool iframe, see Shadownet as the default rehearsal network, see the wtfOS IPFS provider, connect a Shadownet puppet wallet without RPC errors, show the Beacon picker with Kukai and Temple options on explicit connect, and route selected Kukai pairing to the Shadownet Kukai app; a regular signed-in puppet loading the static Studio does not see the wtfOS IPFS provider; generated mint pages expose clean disconnect, wallet balance/cost status, max-per-wallet status, owned-mint recovery hooks, and bounded theme styling instead of arbitrary stored CSS.",
    durableSideEffectAssertion:
      "The focused runner seeds dummy users and Shadownet puppet wallet metadata, verifies the live Shadownet RPC chain id `NetXsqzbfFenSTS` in the Macaroni iframe, proves the IPFS provider selector follows `trusted_market_creator` access, proves a mismatched RPC is blocked before wallet signing, confirms explicit connect does not open a wallet handoff before the Beacon picker is visible, checks from `/tools/macaroni` that Beacon's real Kukai option can escape the sandbox and load `https://shadownet.kukai.app` instead of a blank, mainnet, or Temple-only tab, and keeps source-policy coverage for the generated mint page's validated wallet restore, disconnect, balance preflight, max-per-wallet, TzKT-owned-mint lookup, and CSS theme allowlist paths.",
  },
  {
