...
const sourceDir = path.join(repoRoot, "public/creation-tools/lasagna");
const iconSourceRoot = path.join(repoRoot, "public/pasta-icons");
const outDir = path.join(appDir, "lasagna");
...
if (!existsSync(sourceDir)) {
  console.error(`Lasagna source assets not found: ${sourceDir}`);
  process.exit(1);
}
...
for (const rel of required) {
  ...
  if (!existsSync(target) || statSync(target).size === 0) {
    console.error(`Missing Lasagna desktop asset: ${rel}`);
    process.exit(1);
  }
}
...
console.log(`Prepared Lasagna desktop assets in ${path.relative(repoRoot, outDir)}`);
