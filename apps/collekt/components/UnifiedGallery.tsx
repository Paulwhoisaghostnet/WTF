...
import { isWtfProfileGallery } from "@/lib/wtf/profile-tokens";
...
export default function UnifiedGallery({
    ...
    const isWtfProfile = isWtfProfileGallery(address);
    ...
    useEffect(() => {
        ...
        const fetchNFTsForPage = async () => {
            try {
                ...
                if (currentPage > result.pagination.totalPages && result.pagination.totalItems > 0) {
                    router.push(isWtfProfile ? "/wtf" : `/gallery/${address}`); // Go back to page 1
                    return;
                }
        ...
        fetchNFTsForPage();
    }, [address, currentPage, isBasePage, galleryType, isWtfProfile]);
