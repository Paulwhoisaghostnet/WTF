...
export const PASTA_WTFME_NETWORK = {
  key: "shadownet",
...
export const PASTA_WTFME_PROOF_CONTRACTS = [
  {
    app: "spaghetti",
    kind: "collection",
    ...
    tokenId: "0",
    relationshipGroup: "spaghetti-shadownet-e2e-mr19mwvk",
    route: "/collection",
  },
  {
    app: "gnocchi",
    ...
    tokenId: "0",
    relationshipGroup: "gnocchi-shadownet-e2e-mr1aacew",
    route: "/mint",
    mintEntrypoint: "open_mint",
    priceMutez: "1",
  },
  {
    app: "ravioli",
    kind: "bundle",
    title: "Ravioli Proof Bundle",
    contract: "KT1CeJYHodXy8dvmNFgXxk4zh6SjVB5KYLaG",
    tokenId: "0",
    relationshipGroup: "ravioli-shadownet-e2e-mr1ano0u",
    route: "/",
  },
  {
    app: "rotini",
    kind: "generative",
    title: "Rotini Proof Seeds",
    contract: "KT1SHHPFkthiSTf9CAmhAzWmbi7t5rTcUeYz",
    tokenId: "1",
    relationshipGroup: "rotini-shadownet-e2e-mr1b70wd",
    route: "/",
  },
  {
    app: "penne",
    kind: "distribution",
    title: "Penne Proof Distribution Token",
    contract: "KT1DDY9Pyr7PYNJgXxnHnJn9T7WHaVx7ztdx",
    tokenId: "0",
    relationshipGroup: "penne-shadownet-e2e-mr1bvphs",
    route: "/",
  },
  {
    app: "lasagna",
    ...
    relationshipGroup: "lasagna-shadownet-e2e-mr1caxn6",
    route: "/",
  },
] as const;

type PastaProofContract = (typeof PASTA_WTFME_PROOF_CONTRACTS)[number];
...
function proofAttrs(pageKind: string, contract?: PastaProofContract): string {
  ...
  if (contract) {
    ...
    if ("tokenId" in contract && contract.tokenId) attrs["data-pasta-token-id"] = contract.tokenId;
    if ("mintEntrypoint" in contract && contract.mintEntrypoint) {
      attrs["data-pasta-mint-entrypoint"] = contract.mintEntrypoint;
    }
    if ("priceMutez" in contract && contract.priceMutez) attrs["data-pasta-price-mutez"] = contract.priceMutez;
  }
...
function pageFrame(pageKind: string, title: string, contract: PastaProofContract | null, body: string): string {
...
function landingPage(): ManifestPageSnapshot {
  const cards = PASTA_WTFME_PROOF_CONTRACTS.map((contract) => {
    const token = "tokenId" in contract && contract.tokenId ? ` token ${escapeHtml(contract.tokenId)}` : "";
...
function mintPage(): ManifestPageSnapshot {
  ...
  if (!contract || !("mintEntrypoint" in contract)) throw new Error("Missing Gnocchi mint proof contract");
  return {
    slug: "mint",
