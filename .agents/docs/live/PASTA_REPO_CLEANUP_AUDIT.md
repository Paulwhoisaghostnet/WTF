...
- Authoritative production source is current `origin/main` at `cb74cc13f1a0e4cb05d99b68394a553c0a03321d` (`Add Inbox compose and conversation replies`).
- Live `https://wtfos.app/api/health` returned `status: "ok"`, `version.commitRef: "cb74cc1"`, `nodeEnv: "production"`, healthy database, and current mainnet chain defaults after the latest main deployment. The unified Pasta installer catalog remains deployed and auth-protected on this live commit.
- Clean promotion branch `codex/pasta-installer-audit-docs` was merged into `main` for the scoped Pasta installer/readiness evidence fix. PR #13 promoted the GitHub Actions API workflow-list audit fix plus refreshed live evidence docs. PR #14 then promoted the read-only WTF.ME inventory/readiness hardening, repo cleanup gate integration, and unified installer-catalog readiness hardening. The current `codex/pasta-live-verification-docs` branch records post-PR #14 evidence and fixes the cleanup audit's active-branch/remote classification.
- Historical Pasta release milestones remain ancestors of `cb74cc1`: Gnocchi/Ravioli/Rotini/Penne/Lasagna standalone installer lane commit `0558be0`, Spaghetti installer route commit `09fff2fb`, Spaghetti live verification `e080b89`, stale-worktree cleanup `21e1aca`, Shadownet preflight `24cf9e2`, standalone audit fix `51ab323`, and installer catalog gate `af9461f`.
- Deploy to Hetzner run `28529083020` and main Quality Gates run `28529082794` passed for `cb74cc13`. The prior deploy `28527900056` and Quality Gates run `28527900086` passed for `04b242a`, where the installer catalog first reached live production.
...
- 2026-07-01 follow-up verification from the clean `cb74cc1` live state passed `npm run pasta:live-readiness:check`, `npm run pasta:standalone-installers:audit:check`, `npm run pasta:repo-cleanup:audit:check`, `npm run pasta:repo-cleanup:audit`, and `PASTA_LIVE_READINESS_ALLOW_BLOCKERS=1 npm run pasta:live-readiness`; the latter proves the unified `/api/pasta/installers/catalog` route is live and auth-protected with unauthenticated `401`, verifies all individual/suite installer release assets, verifies the recorded Colander action proof, and blocks only on missing dedicated WTF.ME publish credentials plus missing live Pasta WTF.ME host proof.
- 2026-07-01 cleanup-gate follow-up added `npm run pasta:repo-cleanup:audit` plus `npm run pasta:repo-cleanup:audit:check`; the audit now machine-classifies Pasta-relevant branches/worktrees as active, promoted ancestors, or historical evidence unsafe to replay while ignoring Gamma/Beta noise. The current branch also fixes the audit to treat `origin/<current-branch>` as valid ongoing work and to let the already-merged `codex/pasta-readiness-catalog-live` branch classify as a promoted ancestor. `npm run pasta:live-readiness` invokes this audit by default so release-readiness runs fail on unclassified Pasta branch/worktree state unless explicitly skipped with `PASTA_LIVE_READINESS_CHECK_REPO_CLEANUP=0`.
...
| `.config/superpowers/worktrees/WTF/codex-spaghetti-installer-live` | `codex/pasta-live-verification-docs` / current branch head | current PR delta | Current live verification and cleanup-audit fix checkout | Use as the clean proof base for post-PR #14 evidence updates and cleanup-audit active-remote classification. The implementation delta from `codex/pasta-readiness-catalog-live` is already promoted to `origin/main`; full Pasta remains blocked by live WTF.ME host and pin-discovery proof, not by individual installer publication or catalog deployment. |
...
| --- | ---: | --- | --- |
| `codex/pasta-readiness-catalog-live` | ancestor/current main via PR #14 | Promoted Pasta catalog hardening | Keep as merge history only; do not continue new work there unless comparing against current `origin/main`. |
| `codex/pasta-installer-audit-docs` | ancestor/current main | Promoted PR #13 history | Keep only as merge history for the standalone installer audit evidence fix; do not continue new work there. |
| `codex/spaghetti-installer-live` / `origin/codex/spaghetti-installer-live` | `3 0` (`origin/main` ahead / branch ahead) | Fully promoted ancestor | Archive/delete after user confirmation. It has no commits ahead of `origin/main`; the current mainline contains the standalone installer audit follow-up. |
| `codex/pasta-live-readiness` / `origin/codex/pasta-live-readiness` | `39 22` | Historical proof branch, unsafe to replay | Keep only as evidence. `origin/main..codex/pasta-live-readiness` touches 118 files with 28,795 deletions, including deletion of newer Gnocchi/Ravioli/Rotini/Penne/Lasagna desktop packages and workflows, `pasta:live-readiness`, standalone installer audit tooling, Colander action proof scripts, and published proof reports. Mine individual ideas only by comparing current main first. |
| `wip/full-working-tree-20260617` | `188 2` | Monolithic stale backup | Do not merge. It is two old backup commits over a much older base; replay would touch 807 files with 194,914 deletions, including removal of current installer apps, workflows, Pasta reports, spec files, and live readiness tooling. Archive after user confirmation. |
| `codex/ipfs-pinning-organ` | ancestor | Promoted IPFS history | Archive candidate after user confirmation; no commits ahead of current main. |
| `codex/macaroni-access-export`, `codex/macaroni-browser-compat`, `codex/macaroni-direct-upload-lane`, `codex/macaroni-drop-repair`, `codex/macaroni-layout-audit`, `codex/macaroni-new-drop-controls`, `codex/macaroni-onboarding-guards`, `codex/macaroni-practical-media-limits`, `codex/macaroni-recent-mints`, `codex/macaroni-v2-full-send` | ancestor | Promoted Macaroni history | Archive candidates after user confirmation; all are ancestors of current main and not needed for live Pasta installer deployment. |
...
Executable audit proof: `npm run pasta:repo-cleanup:audit` passed with `ok: true`, base `origin/main` at `cb74cc13f1a0e4cb05d99b68394a553c0a03321d`, active ref `codex/pasta-live-verification-docs`, no blockers, the local and remote active branch classified as `valid_ongoing_work`, `codex/pasta-readiness-catalog-live` / `origin/codex/pasta-readiness-catalog-live` classified as `promoted_ancestor`, `codex/pasta-live-readiness` / `origin/codex/pasta-live-readiness` classified as `historical_evidence_unsafe_to_replay`, and promoted Macaroni/IPFS/Spaghetti branches classified as `promoted_ancestor`.
...
- The former `codex/pasta-live-readiness` proof branch has been mined/reworked into narrower release lanes. Future work should continue from the current `codex/pasta-live-verification-docs` branch while this PR is active, or from a fresh branch from `origin/main`; do not continue from the older broad proof branch, the merged `codex/pasta-installer-audit-docs` branch, the promoted `codex/pasta-readiness-catalog-live` branch, or the now-promoted `codex/spaghetti-installer-live` branch.
