...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (typeof body.sourceMode === "string" && ["all_users", "selected_users", "specific_wallets"].includes(body.sourceMode)) {
        fields.sourceMode = body.sourceMode;
      }
      if (Array.isArray(body.sourceUserIds)) fields.sourceUserIds = body.sourceUserIds;
      if (Array.isArray(body.sourceWalletAddresses)) fields.sourceWalletAddresses = body.sourceWalletAddresses;
      if (typeof body.tokensPerWalletPerHour === "number") {
        fields.tokensPerWalletPerHour = Math.max(1, Math.min(100, Math.floor(body.tokensPerWalletPerHour)));
      }
      if (typeof body.defaultDurationSeconds === "number") {
        fields.defaultDurationSeconds = Math.max(3, Math.min(300, Math.floor(body.defaultDurationSeconds)));
      }
      if (typeof body.playlistSize === "number") {
        fields.playlistSize = Math.max(5, Math.min(500, Math.floor(body.playlistSize)));
      }
      if (typeof body.refreshIntervalMinutes === "number") {
        fields.refreshIntervalMinutes = Math.max(5, Math.min(1440, Math.floor(body.refreshIntervalMinutes)));
      }
      if (typeof body.bumperMode === "string" && ["community_pool", "selected", "none"].includes(body.bumperMode)) {
        fields.bumperMode = body.bumperMode;
      }
      if (Array.isArray(body.selectedBumperIds)) fields.selectedBumperIds = body.selectedBumperIds;
  ...
  async (req, res) => {
    try {
      ...
      if (existing?.channelId) {
        ...
        if (ch) return res.json({ config: existing, message: "WTF TV channel already exists" });
      }
      ...
      const [channel] = await db
        ...
        .values({
          ownerUserId: user.id,
          title: "WTF TV",
          description: "The official WTF community channel — random tokens from the community, 24/7.",
          slug: "wtf-tv",
          isActive: true,
        })
        .returning();
...
router.post(
  ...
  async (_req, res) => {
    try {
      const { refreshWtfPlaylist } = await import("./tv");
