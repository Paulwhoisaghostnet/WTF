...
- Clean promotion branch `codex/spaghetti-installer-live` is currently the Pasta release lane and ahead of `origin/main` by 22 scoped commits after this pass: WTF.ME hosted-page proof, Pasta pinning/recovery proof, hardened pin recovery publish path, suite manifest `bundledApps`, explicit-host WTF.ME live check, explicit/discovered-host live inventory, stale-branch classification refresh, live publisher overwrite guards, live publisher expected-host pinning, a Pasta live-readiness gate, installer-aware readiness coverage, post-publish public host verification, credentialed publish dry-run validation, signer-backed Colander management action proof, Colander browser-wallet choreography proof, non-spending recorded Colander action verification inside the readiness gate, public manifest-payload verifier wiring, and standalone installer source plumbing for Gnocchi, Ravioli, Rotini, Penne, and Lasagna.
...
| `.config/superpowers/worktrees/WTF/codex-spaghetti-installer-live` | `codex/spaghetti-installer-live` | 0 | Current Pasta production/promotion checkout | Clean continuation lane, ahead of `origin/main` by 22 scoped commits after this pass. This is the only Pasta release branch to promote from; still blocked from a full production Pasta claim by live WTF.ME host and pin-discovery proof plus individual installer release publication for Gnocchi/Ravioli/Rotini/Penne/Lasagna. |
...
6. Publish and configure the Gnocchi, Ravioli, Rotini, Penne, and Lasagna standalone installer release assets, then make each authenticated live verifier pass. Decide whether Colander should ship as a separate native admin app or remain an in-suite/main-app surface.
