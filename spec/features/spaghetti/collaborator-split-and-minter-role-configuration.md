# Spaghetti: Collaborator, split, and minter role configuration

## Purpose

Collaborator, split, and minter role configuration lets Pasta Protocol users move the Spaghetti workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- The user can access `/tools/spaghetti` and the relevant app gate is enabled.
...
- User-entered configuration for collaborator, split, and minter role configuration.
- Wallet/network context when the feature signs or reads chain state.
- Package, media, metadata, contract, or relationship data appropriate to Spaghetti.

## Outputs

- Validated state for collaborator, split, and minter role configuration.
...
- Standalone validation: open Spaghetti, exercise collaborator, split, and minter role configuration, assert visible status and no console-modal dependency.
