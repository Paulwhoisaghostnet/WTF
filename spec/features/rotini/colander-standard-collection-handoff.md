# Rotini: Colander standard-collection handoff

## Purpose

Colander standard-collection handoff lets Pasta Protocol users move the Rotini workflow toward a valid contract product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- The user can access `/tools/rotini` and the relevant app gate is enabled.
...
- User-entered configuration for colander standard-collection handoff.
- Wallet/network context when the feature signs or reads chain state.
- Package, media, metadata, contract, or relationship data appropriate to Rotini.

## Outputs

- Validated state for colander standard-collection handoff.
...
- Standalone validation: open Rotini, exercise colander standard-collection handoff, assert visible status and no console-modal dependency.
