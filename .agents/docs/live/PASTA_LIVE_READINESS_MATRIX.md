...
1. Provision or identify a Pasta-specific production user whose username is a DNS-valid WTF.ME label and whose account has wallet plus OAuth social, linked Bluesky, or active WTF DID state; then run `npm run pasta:wtfme:live-publish` in dry-run mode to confirm the exact eligible host.
