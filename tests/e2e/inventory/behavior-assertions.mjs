export const CORE_BEHAVIOR_ASSERTIONS = [
  ...
  },
  {
    ...
    userVisibleAssertion:
      "A trusted-creator puppet can open Macaroni, enter the Studio through the embedded creation-tool iframe, see Shadownet as the default rehearsal network, see the wtfOS IPFS provider, see Fileship as the default IPFS gateway, see the 250 MB artifact and 1 MB square JPG/PNG collection logo/cover limits, connect a Shadownet puppet wallet without RPC errors, show the Beacon picker with Kukai and Temple options on explicit connect, route selected Kukai pairing to the Shadownet Kukai app, receive sandbox-safe inline validation/deploy feedback instead of blocked browser modals while Studio keeps deploy/sync signing on the same chain-guarded path for Shadownet and mainnet, clear the current Studio draft into a new drop without disconnecting the wallet, restore a project from backup JSON, and visibly save sale-stage edits; a regular signed-in puppet loading the static Studio does not see the wtfOS IPFS provider; generated mint pages expose clean disconnect, prevent duplicate request-permission flows from rapid connect clicks, include basic accessibility landmarks/status/progress/quantity semantics, clamp requested mint quantity to live collection remaining supply plus the connected wallet's remaining per-wallet/allowlist allowance, show wallet balance/cost status, max-per-wallet status, owned-mint recovery hooks, and bounded theme styling instead of arbitrary stored CSS.",
    durableSideEffectAssertion:
      "The focused runner seeds dummy users and Shadownet puppet wallet metadata, verifies the live Shadownet RPC chain id `NetXsqzbfFenSTS` in the Macaroni iframe, proves the IPFS provider selector follows `trusted_market_creator` access, proves a mismatched RPC is blocked before wallet signing, confirms explicit connect does not open a wallet handoff before the Beacon picker is visible, checks from `/tools/macaroni` that Beacon's real Kukai option can escape the sandbox and load `https://shadownet.kukai.app` instead of a blank, mainnet, or Temple-only tab, asserts rapid generated-page connect clicks coalesce to one Beacon permission request, and keeps source-policy coverage for OBJKT media limits, sandbox-safe Studio notices without native `alert`/`confirm`, shared mainnet/Shadownet deploy-path parity, optional treasury/royalty address normalization, new-drop draft reset, backup JSON restore, sale-stage save confirmation, non-image cover preview metadata, the generated mint page's validated wallet restore, disconnect, accessible controls/status regions, balance preflight, max-per-wallet and allowlist remaining allowance clamping, TzKT-owned-mint lookup, Fileship gateway default, and CSS theme allowlist paths.",
  },
  {
