...
The beta home now adds a first-screen OS session console inside the hero and a playable desk before the long-form material. The session console gives users one dominant Launch Quest command plus Play/People/Return launcher states before status detail, the visual world-stage pulse proves other people/objects/return hooks exist without creating another row of website CTAs, and the desk gives the selected user one current quest, proof route, return route, EXP progress, five world lanes for quest/people/discovery/tomorrow/Count, visible 60-second answers, and a deliberate Research Deck opener. This makes the default experience feel like an operating shell instead of a Wikipedia-style evidence page.
