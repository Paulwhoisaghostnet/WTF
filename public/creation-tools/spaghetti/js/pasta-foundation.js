...
export function parseRecipientList(text, defaultAmount = 1) {
  const entries = [];
  const errors = [];
  const seen = new Map();
...
export function totalAllocation(entries) {
  return entries.reduce((sum, e) => sum + (Number.isFinite(e.amount) ? e.amount : 0), 0);
}

// ---------- exhibition references + metadata (Lasagna) ----------
...
export function buildExhibitionMetadata(input) {
  const base = {
    name: input.name,
...
// ---------- deterministic generative engine (Rotini) ----------
...
export function pickVariantIndex(variants, r) {
