# WtfOpenEditionFA2 - SmartPy v2
# Port of BowersOpenEditionFA2 under the WTF Collection umbrella.
#
# Changes vs. the Bowers source:
#   * Renamed to Wtf* so Objkt indexer groups mints from all four WTF modes
#     under a single collection identity via the shared `collection_id`
#     stored in `data.collection_metadata`.
#   * `collection_metadata` big_map exposes a stable key (``) → bytes payload
#     that resolves to an ipfs:// JSON describing the umbrella collection.
#     Objkt's indexer reads this for collection grouping; keeping the shape
...
#     admin address.
#
# FA2 multi-asset with open edition token factories. Admin creates tokens
# via `create_token`, public mints via `mint_editions`. Full marketplace:
# listings, buy, offers (partial fill), blacklist, withdraw. Per-token
# royalty and min offer stored in `token_config`.
...
def main():
    ...
    class WtfOpenEditionFA2(sp.Contract):
        ...
        # ---- Open edition: create_token, mint_editions ----
        ...
        def create_token(self, params):
            ...
            sp.emit(sp.record(token_id=tid, creator=params.creator, price=params.mint_price), tag="token_created")
...
@sp.add_test()
def test():
    scenario = sp.test_scenario("WtfOpenEdition", main)
    admin = sp.test_account("admin")
    alice = sp.test_account("alice")
    bob = sp.test_account("bob")

    c = main.WtfOpenEditionFA2(
        admin=admin.address,
    ...
    c.create_token(
        metadata_uri=bytes_of_string("ipfs://QmExampleMetadataCID"),
    ...
    c.set_mint_paused(token_id=0, paused=True, _sender=admin)
    c.set_mint_paused(token_id=0, paused=False, _sender=admin)
    c.mint_editions(token_id=0, qty=1, to_=bob.address, _sender=bob, _amount=sp.tez(1))
