...
const PRODUCT_NAME = "Ravioli Studio";
...
function ravioliRoot() {
  return path.join(appRoot(), "ravioli");
}
...
function safeStaticPath(urlPath) {
  ...
  const fullPath = path.join(ravioliRoot(), rel);
  const root = ravioliRoot();
...
async function handleRequest(req, res) {
  ...
  if (req.method === "GET" && parsed.pathname === "/api/auth/user") {
    return sendJson(res, 401, {
      error: "Ravioli Desktop does not include wtfOS hosted resources.",
      native: true,
      product: "ravioli-studio",
    });
  }
  if (req.method === "GET" && parsed.pathname === "/api/auth/csrf-token") {
    return sendJson(res, 200, { csrfToken: "ravioli-desktop" });
  }
  if (req.method === "GET" && parsed.pathname === "/api/ravioli/installers") {
  ...
  if (
    ...
    return sendJson(res, 403, {
      error: "wtfOS hosted pinning, publishing, and preview processing are not available in Ravioli Desktop. Use Pinata or your own IPFS node.",
      native: true,
      product: "ravioli-studio",
    });
  }
...
if (!hasSingleInstanceLock) {
  ...
  app.whenReady().then(async () => {
    try {
      ...
      dialog.showErrorBox("Ravioli Studio failed to start", err.message || String(err));
      app.quit();
    }
