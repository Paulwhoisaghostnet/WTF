# Ravioli: Bundle token minting

## Purpose

Bundle token minting lets Pasta Protocol users move the Ravioli workflow toward a valid token product outcome while preserving wallet, pinning, relationship metadata, and recovery guarantees.
...
- User-entered configuration for bundle token minting.
...
- Validated state for bundle token minting.
...
- Standalone validation: open Ravioli, exercise bundle token minting, assert visible status and no console-modal dependency.
